...
async function initializeDatabase() {
    try {
        ...
        try {
            ...
            db = null;
            console.error('[DB ERROR] Connexion directe échouée:', dbError.message);
        ...
        if (db) {
            console.log('[SUCCESS] MySQL Connected to', DB_CONFIG.database);

            // S'assurer que la table hero_slides existe
            try {
                await db.execute(`
            ...
            `);
                console.log('[SUCCESS] Table hero_slides OK');
            } catch (e) {
                console.error('[ERROR] Creation table hero_slides a echoue:', e.message);
            }
            ...
            console.log('[WARNING] Database initialized in disconnected mode');
        }
    } catch (error) {
...
apiRouter.get('/statistics', async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(
            'SELECT * FROM active_statistics ORDER BY id'
        );
        res.json(rows);
    } catch (error) {
...
app.put('/api/statistics/:key', async (req, res) => {
    try {
        const { key } = req.params;
        const { value, label, suffix, color, icon, is_active } = req.body;

        console.log('[STATISTICS] Creating/updating:', key, { value, label, suffix, color, icon, is_active });

        // D'abord essayer INSERT simple
        try {
            ...
            console.log('[STATISTICS] INSERT success:', result);
        } catch (insertError) {
            // Si INSERT échoue (clé existe déjà), faire UPDATE
            console.log('[STATISTICS] INSERT failed, trying UPDATE:', insertError.message);
            const updateResult = await db.execute(
                `UPDATE statistics SET value = ?, label = ?, suffix = ?, color = ?, icon = ?
                 WHERE stat_key = ?`,
                [value, label || value, suffix || '', color || 'blue', icon || 'FaChartBar', key]
            );
            console.log('[STATISTICS] UPDATE result:', updateResult);
        }

        console.log('[STATISTICS] DB result:', result);
...
apiRouter.get('/testimonials', async (req, res) => {
    try {
        ...
        const admin = req.query.admin === '1';
        const [rows] = await db.execute(
            admin ? 'SELECT * FROM testimonials ORDER BY sort_order' : 'SELECT * FROM active_testimonials ORDER BY sort_order'
        );
