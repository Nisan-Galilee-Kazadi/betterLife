...
import { FaHeart, FaChartLine, FaUsers, FaGraduationCap } from 'react-icons/fa'
import { GiBeehive, GiHoneyJar } from 'react-icons/gi'
...
export function OffreApiculture() {
    const { t } = useLanguage()

    // Helper to get packages array
    const packages = [
        { ...t('offres.apiculture.packages.items.0'), popular: false },
        { ...t('offres.apiculture.packages.items.1'), popular: true },
        { ...t('offres.apiculture.packages.items.2'), popular: false }
    ]
            ...
            <div className="relative isolate overflow-hidden bg-gradient-to-br from-[#63b32e] to-[#0f70b7] py-24 sm:py-32">
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    <div className="mx-auto max-w-2xl text-center">
                        <p className="text-sm font-semibold uppercase tracking-widest text-white/80 mb-4">{t('offres.apiculture.hero.tag')}</p>
                        <h1 className="text-4xl font-bold tracking-tight text-white sm:text-6xl">
                            {t('offres.apiculture.hero.title')}
                        </h1>
                        <p className="mt-6 text-lg leading-8 text-white/90">
                            {t('offres.apiculture.hero.subtitle')}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="mx-auto max-w-7xl px-6 py-16 sm:py-24">
                ...
                <div className="mb-20">
                    <div className="text-center mb-12">
                        <SectionTitle kicker={t('offres.apiculture.packages.kicker')} title={t('offres.apiculture.packages.title')} />
                    </div>
                        ...
                        {packages.map((pkg, index) => (
                            <div key={index} className={`rounded-3xl p-8 ${pkg.popular ? 'ring-4 ring-[#0f70b7] shadow-2xl scale-105' : 'ring-1 ring-slate-200 shadow-lg'} bg-white relative`}>
                                ...
                                <div className="mb-8 hidden">
                                    {/* Pricing will be dynamic from translation if needed, but currently keeping it hidden or we can map it too if we add it to the objects */}
                ...
                <div className="mb-20">
                    <div className="text-center mb-12">
                        <SectionTitle kicker={t('offres.apiculture.products.kicker')} title={t('offres.apiculture.products.title')} />
                    </div>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {t('offres.apiculture.products.items').map((item, index) => {
                            const Icon = productIcons[index] || GiBeehive
                            return (
                ...
                <div className="mb-20 rounded-3xl bg-gradient-to-br from-green-50 to-blue-50 p-12 border-2 border-green-200">
                    <h2 className="text-3xl font-bold text-slate-900 text-center mb-12">{t('offres.apiculture.benefits.title')}</h2>
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {t('offres.apiculture.benefits.items').map((benefit, index) => {
                            const Icon = benefitIcons[index] || FaChartLine
                            return (
                ...
                <div className="rounded-3xl bg-gradient-to-r from-[#63b32e] to-[#0f70b7] p-12 text-center">
                    <GiBeehive className="text-7xl text-white mx-auto mb-6" />
                    <h2 className="text-3xl font-bold text-white mb-4">
                        {t('offres.apiculture.cta.title')}
                    </h2>
                    <p className="text-xl text-white/90 mb-8 max-w-2xl mx-auto">
                        {t('offres.apiculture.cta.text')}
                    </p>
                    <Link
                        to="/contact"
