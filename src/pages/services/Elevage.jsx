...
import { GiFarmer, GiChicken, GiCow, GiPig, GiGoat, GiBeehive, GiFishEggs } from 'react-icons/gi'
...
export function ServiceElevage() {
    ...
    const livestockIcons = [GiChicken, GiGoat, GiPig, GiCow, GiBeehive, GiFishEggs]
                ...
                <div className="mb-20">
                    ...
                    <div className="grid gap-8 md:grid-cols-2">
                        {t('services.elevage.services.items').map((service, index) => {
                            ...
                            return (
                                <div key={index} className="rounded-xl border-2 border-green-200 bg-green-50 p-8">
                        ...
                        <div className="grid gap-12 lg:grid-cols-2 items-center">
                                ...
                                <div className="space-y-4">
                                    {t('services.elevage.story.benefits').map((item, index) => (
                                        <div key={index} className="flex items-center gap-3">
                                            <svg className="w-6 h-6 text-[#63b32e]" fill="currentColor" viewBox="0 0 20 20">
                                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                                            </svg>
                                            <span className="text-slate-700">{item}</span>
                                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                {t('services.elevage.story.stats').map((stat, index) => (
                                    <div key={index} className={`rounded-xl border-2 ${stat.color === 'green' ? 'border-green-200 bg-green-50' : 'border-blue-200 bg-blue-50'} p-6 text-center`}>
                                        <p className="text-sm text-slate-600 mb-2">{stat.label}</p>
                                        <p className={`text-3xl font-bold ${stat.color === 'green' ? 'text-[#63b32e]' : 'text-[#0f70b7]'}`}>
                                            {stat.value}
                                        </p>
                                    </div>
                                ))}
