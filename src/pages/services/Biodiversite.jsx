...
import { GiElephant, GiGorilla, GiButterfly, GiForest, GiSwamp } from 'react-icons/gi'
            ...
            <div className="mx-auto max-w-7xl px-6 py-16 sm:py-24">
                ...
                <div className="mb-20">
                    ...
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                        {t('services.biodiversite.species.items').map((species, index) => {
                            ...
                            const bgClass = isGreen ? 'border-green-200 bg-green-50' : 'border-blue-200 bg-blue-50'

                            return (
                                <div key={index} className={`rounded-2xl border-2 ${bgClass} p-6 text-center`}>
                                    <Icon className={`text-6xl mx-auto mb-4 ${colorClass}`} />
                                    <h3 className="font-bold text-slate-900 mb-2">{species.name}</h3>
                ...
                <div className="mb-20">
                    ...
                    <div className="grid gap-8 md:grid-cols-2">
                        {t('services.biodiversite.programs.items').map((program, index) => {
                            ...
                            return (
                                <div key={index} className="rounded-2xl bg-white p-8 shadow-lg ring-1 ring-slate-900/5">
                ...
                <div className="mb-20">
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                        {t('services.biodiversite.ecosystems.items').map((ecosystem, index) => {
                            const Icon = ecosystemIcons[index] || GiForest
                            return (
                                <div key={index} className="rounded-xl bg-gradient-to-br from-green-50 to-blue-50 border-2 border-green-200 p-6">
                                    <Icon className="text-6xl text-[#63b32e] mb-4" />
                                    <h3 className="text-xl font-bold text-slate-900 mb-4">{ecosystem.name}</h3>
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <p className="text-sm text-slate-600">Surface</p>
                                            <p className="text-lg font-bold text-[#0f70b7]">{ecosystem.area}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-slate-600">Espèces</p>
                                            <p className="text-lg font-bold text-[#0f70b7]">{ecosystem.species}</p>
                                        </div>
                                    </div>
