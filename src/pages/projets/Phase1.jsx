...
import { FaUsers, FaHandsHelping, FaGraduationCap, FaChartLine } from 'react-icons/fa'
            ...
            <div className="mx-auto max-w-4xl px-6 py-20">
                ...
                <div className="space-y-8">
                    <div className="rounded-2xl bg-blue-50 border-2 border-blue-200 p-8">
                        <h2 className="text-2xl font-bold text-slate-900 mb-4">{t('projets.phase1.objectives.title')}</h2>
                        <ul className="space-y-3">
                            {objectives.map((obj) => (
                                <li key={obj} className="flex items-start gap-3">
                                    <svg className="w-6 h-6 text-[#0f70b7] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <div className="rounded-xl bg-green-50 border border-green-200 p-6">
                            <h3 className="font-bold text-slate-900 mb-3">{t('projets.phase1.stats.duration.label')}</h3>
                            <p className="text-3xl font-bold text-[#63b32e]">{t('projets.phase1.stats.duration.value')}</p>
                        </div>
                        <div className="rounded-xl bg-blue-50 border border-blue-200 p-6">
                            <h3 className="font-bold text-slate-900 mb-3">{t('projets.phase1.stats.budget.label')}</h3>
                            <p className="text-3xl font-bold text-[#0f70b7]">{t('projets.phase1.stats.budget.value')}</p>
                            <p className="text-sm text-slate-600 mt-1">{t('projets.phase1.stats.budget.subtext')}</p>
                        </div>
                    </div>

                    <div className="rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50 p-8 border-2 border-slate-200">
                        <h2 className="text-2xl font-bold text-slate-900 mb-6">{t('projets.phase1.activities.title')}</h2>
