...
import { FaCheckCircle, FaUsers, FaDollarSign, FaTree, FaHandsHelping, FaChartLine, FaCalendar, FaMapMarkedAlt } from 'react-icons/fa'
import { MdTrendingUp } from 'react-icons/md'
import { useLanguage } from '../../translations/context/LanguageContext'
            ...
            <div className="mx-auto max-w-7xl px-6 py-16 sm:py-24">
                ...
                <div className="mb-20">
                    <div className="text-center mb-12">
                        <SectionTitle kicker={t('projets.realises.impact.kicker')} title={t('projets.realises.impact.title')} />
                    </div>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                        {impactStats.map((stat, index) => (
                            <div key={index} className={`rounded-2xl border-2 ${stat.color === 'green' ? 'border-green-200 bg-green-50' : 'border-blue-200 bg-blue-50'} p-6 text-center`}>
                                <stat.icon className={`text-6xl mb-3 mx-auto ${stat.color === 'green' ? 'text-[#63b32e]' : 'text-[#0f70b7]'}`} />
                                <p className="text-4xl font-bold text-[#63b32e] mb-2">{stat.number}</p>
                ...
                <div className="space-y-16">
                    {completedProjects.map((project, index) => (
                        <div key={index} className="rounded-3xl bg-gradient-to-br from-slate-50 to-green-50 p-8 border-2 border-green-200">
                            {/* Project Header */}
                            ...
                            <div className="grid gap-4 md:grid-cols-4 mb-8">
                                {project.impact && Object.entries(project.impact).map(([key, value]) => (
                                    <div key={key} className="rounded-xl bg-white p-4 text-center shadow-sm">
                                        <p className="text-2xl font-bold text-[#63b32e] mb-1">{value.split(' ')[0]}</p>
                                        <p className="text-xs text-slate-600">{value.split(' ').slice(1).join(' ')}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mb-8">
                                ...
                                <div className="grid gap-3 md:grid-cols-2">
                                    {project.achievements && project.achievements.map((achievement, i) => (
                            ...
                            {project.testimonial && (
                                <div className="rounded-2xl bg-white p-6 border-l-4 border-[#63b32e]">
                                    <p className="text-slate-700 italic mb-4">"{project.testimonial.quote}"</p>
                                    <p className="text-sm font-semibold text-[#0f70b7]">— {project.testimonial.author}</p>
                                </div>
                            )}
                        </div>
                    ))}
                ...
                <div className="mt-20 mb-20">
                    <div className="text-center mb-12">
                        <SectionTitle kicker={t('projets.realises.factors.kicker')} title={t('projets.realises.factors.title')} />
                    </div>
                    <div className="grid gap-6 md:grid-cols-3">
                        {factors.map((factor, index) => (
                            <div key={index} className="rounded-2xl bg-white p-8 shadow-lg ring-1 ring-slate-900/5 text-center">
