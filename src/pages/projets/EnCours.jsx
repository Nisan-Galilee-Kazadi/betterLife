...
import { FaChartLine, FaUsers, FaDollarSign, FaMapMarkedAlt } from 'react-icons/fa'
            ...
            <div className="mx-auto max-w-7xl px-6 py-16 sm:py-24">
                ...
                <div className="mb-20 grid gap-6 md:grid-cols-4">
                    {stats.map((stat, index) => (
                        <div key={index} className="rounded-xl border-2 border-blue-100 bg-blue-50 p-6 text-center">
                            <stat.icon className="text-5xl mb-2 text-[#0f70b7] mx-auto" />
                            <p className="text-3xl font-bold text-[#0f70b7] mb-1">{stat.number}</p>
                ...
                <div className="space-y-12">
                    {projects.map((project, index) => (
                        <div key={index} className="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-900/5">
                            {/* Project Header */}
                            <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                                <div>
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <Link
                                            to={`/projets/en-cours/${project.phase.toLowerCase().replace(' ', '')}`}
                                ...
                                <div className="text-right">
                                    <p className="text-sm text-slate-600">{t('projets.common.duration')}</p>
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-sm font-semibold text-slate-700">{t('projets.common.progress')}</span>
                                    <span className="text-2xl font-bold text-[#63b32e]">{project.progress}%</span>
                                </div>
                            ...
                            <div className="grid gap-4 md:grid-cols-2 mb-6">
                                <div className="rounded-xl border border-green-200 bg-green-50 p-4">
                                    <p className="text-sm text-slate-600 mb-1">{t('projets.common.budget')}</p>
                                    <p className="text-2xl font-bold text-[#63b32e]">{project.budget}</p>
                                </div>
                                <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                                    <p className="text-sm text-slate-600 mb-1">{t('projets.common.beneficiaries')}</p>
                                    <p className="text-2xl font-bold text-[#0f70b7]">{project.beneficiaries}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid gap-6 lg:grid-cols-2">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                                        <svg className="w-5 h-5 text-[#63b32e]" fill="currentColor" viewBox="0 0 20 20">
