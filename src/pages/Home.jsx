...
import { FaUsers, FaTree, FaGlobeAmericas, FaHandsHelping, FaSeedling, FaHeart, FaQuoteLeft } from 'react-icons/fa'
        ...
        <div className="bg-white">
                ...
                <div className="mx-auto max-w-7xl px-6">
                    <FadeIn>
                        <div className="text-center mb-12">
                            <h2 className="text-3xl font-bold text-slate-900 sm:text-4xl">{t('home.stats.title')}</h2>
                            <p className="mt-4 text-lg text-slate-600">{t('home.stats.subtitle')}</p>
                        </div>
                    </FadeIn>
                    <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                        {[
                            { icon: FaUsers, number: '85000', label: t('common.beneficiaries'), color: 'blue', suffix: '+' },
                            { icon: FaTree, number: '450000', label: t('common.trees'), color: 'green', suffix: '' },
                            { icon: FaGlobeAmericas, number: '28', label: t('common.projects'), color: 'blue', suffix: '' },
                            { icon: FaHandsHelping, number: '5', label: t('common.provinces'), color: 'green', suffix: '' }
                        ].map((stat, index) => (
                            <FadeIn key={stat.label} delay={index * 100}>
                                <div className={`rounded-2xl border-2 ${stat.color === 'green' ? 'border-green-200 bg-white' : 'border-blue-200 bg-white'} p-8 text-center shadow-lg transition hover:scale-105 hover:shadow-2xl`}>
                                    <stat.icon className={`text-6xl mb-4 mx-auto ${stat.color === 'green' ? 'text-[#63b32e]' : 'text-[#0f70b7]'}`} />
            ...
            <div className="mx-auto max-w-7xl px-6 py-16 sm:py-24">
                ...
                <FadeIn>
                    <div className="grid gap-12 lg:grid-cols-2 mb-24">
                        <div className="space-y-6">
                            <SectionTitle kicker={t('home.mission.kicker')} title={t('home.mission.title')}>
                            ...
                            <div className="mt-6 flex gap-6">
                                <div className="flex-1 rounded-2xl border-2 border-green-200 bg-green-50 p-6 shadow-lg">
                                    <p className="font-bold text-[#63b32e] text-4xl">{t('home.mission.stat1_val')}</p>
                                    <p className="mt-2 text-sm font-semibold text-slate-700">{t('home.mission.stat1_label')}</p>
                                </div>
                                <div className="flex-1 rounded-2xl border-2 border-blue-200 bg-blue-50 p-6 shadow-lg">
                                    <p className="font-bold text-[#0f70b7] text-4xl">{t('home.mission.stat2_val')}</p>
                                    <p className="mt-2 text-sm font-semibold text-slate-700">{t('home.mission.stat2_label')}</p>
                                </div>
                            </div>
                        </div>
                        <div className="grid gap-6 sm:grid-cols-2">
                            {[
                                { title: t('home.mission.cards.biodiversity.title'), desc: t('home.mission.cards.biodiversity.desc'), icon: GiForest, link: '/Services/biodiversite' },
                                { title: t('home.mission.cards.agriculture.title'), desc: t('home.mission.cards.agriculture.desc'), icon: MdAgriculture, link: '/Services/agriculture' },
                                { title: t('home.mission.cards.reforestation.title'), desc: t('home.mission.cards.reforestation.desc'), icon: FaSeedling, link: '/Services/reboisement' },
                                { title: t('home.mission.cards.community.title'), desc: t('home.mission.cards.community.desc'), icon: FaHandsHelping, link: '/Services/communautaire' },
                            ].map((card) => (
                                <Link
                                    key={card.title}
                                    to={card.link}
                                ...
                                >
                                    ...
                                    <h3 className="font-bold text-slate-900 group-hover:text-[#63b32e] transition">{card.title}</h3>
                ...
                <FadeIn>
                    <div className="mb-24">
                        <div className="text-center mb-12">
                            <SectionTitle kicker={t('home.projects.kicker')} title={t('home.projects.title')} />
                        </div>
                        <div className="grid gap-8 md:grid-cols-3">
                            {[
                                {
                                    title: t('home.projects.cards.p1.title'),
                                    location: t('home.projects.cards.p1.location'),
                                    impact: t('home.projects.cards.p1.impact'),
                                    status: t('home.projects.cards.p1.status'),
                                    link: '/projets/en-cours'
                                },
                                {
                                    title: t('home.projects.cards.p2.title'),
                                    location: t('home.projects.cards.p2.location'),
                                    impact: t('home.projects.cards.p2.impact'),
                                    status: t('home.projects.cards.p2.status'),
                                    link: '/projets/realises'
                                },
                                {
                                    title: t('home.projects.cards.p3.title'),
                                    location: t('home.projects.cards.p3.location'),
                                    impact: t('home.projects.cards.p3.impact'),
                                    status: t('home.projects.cards.p3.status'),
                                    link: '/projets/en-cours'
                                }
                            ].map((project) => (
                                <div key={project.title} className="rounded-2xl border-2 border-slate-200 bg-white p-6 shadow-lg transition hover:shadow-2xl hover:border-[#63b32e]">
                                    <div className="mb-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${project.status === 'En cours' || project.status === 'Ongoing' || project.status === 'En curso' ? 'bg-blue-100 text-[#0f70b7]' : 'bg-green-100 text-[#63b32e]'
                                            }`}>
                                            {project.status}
                                        </span>
                                    </div>
                                    <h3 className="text-xl font-bold text-slate-900 mb-3">{project.title}</h3>
                                    <p className="text-slate-600 mb-2 text-sm">{project.location}</p>
                                    <p className="text-[#63b32e] font-semibold mb-6">{project.impact}</p>
                                    <Link
                                        to={project.link}
                ...
                <FadeIn>
                    <div className="mb-24">
                        <div className="text-center mb-12">
                            <SectionTitle kicker={t('home.testimonials.kicker')} title={t('home.testimonials.title')} />
                        </div>
                        <div className="grid gap-8 md:grid-cols-2">
                            {[
                                ...
                                <div key={index} className="rounded-2xl border-2 border-green-200 bg-gradient-to-br from-green-50 to-white p-8 shadow-lg">
                                    <FaQuoteLeft className="text-4xl text-[#63b32e] mb-4" />
                                    <p className="text-slate-700 italic mb-6 text-lg">"{testimonial.quote}"</p>
