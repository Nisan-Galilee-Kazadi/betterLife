export const sw = {
  common: {
    readMore: "Soma Zaidi",
    contact: "Wasiliana Nasi",
    donate: "Changia",
    join: "Jiunge",
    search: "Tafuta...",
    searchPlaceholder: "Tafuta kwenye tovuti...",
    resultsFound: "matokeo yamepatikana",
    noResults: "Hakuna matokeo kwa",
    typeToSearch: "Andika angalau herufi 2 kutafuta",
    loading: "Inapakia...",
    year: "Mwaka",
    partners: "Kuwa Mshirika",
    mission: "Dhamira Yetu",
    team: "Timu Yetu",
    role: "Nafasi",
    location: "Mahali",
    status: "Hali",
    impact: "Athari",
    beneficiaries: "Wanufahamu",
    trees: "Miti Iliyopandwa",
    projects: "Miradi Iliyokamilika",
    actions: "Vitendo",
    provinces: "Mikoa Iliyofikiwa",
    prevImage: "Picha iliyopita",
    nextImage: "Picha inayofuata",
    goToImage: "Nenda kwenye picha",
    capacity: "Uwezo",
    species: "Aina",
  },
  nav: {
    home: "Nyumbani",
    about: "Kuhusu",
    mission: "Dhamira Yetu",
    team: "Timu Yetu",
    partners: "Washirika Wetu",
    Actions: "Vitendo Vyetu",
    agriculture: "Kilimo",
    cacao: "Kakao (Criollo)",
    cafe: "Kahawa ya Arabica",
    the: "Chai",
    coton_caoutchouc: "Pamba & Mpira",
    arboriculture: "Matunda",
    plantes_medicinales: "Mimea ya Dawa",
    biodiversity: "Bioanuwai",
    community: "Mradi wa Jamii",
    breeding: "Ufugaji",
    elevage_gros_betail: "Mifugo Mikubwa",
    pisciculture: "Ufugaji wa Samaki",
    elevage_chiens: "Ufugaji wa Mbwa",
    elevage_serpents: "Ufugaji wa Nyoka",
    apiculture: "Ufugaji wa Nyuki",
    projet_communautaire: "Mradi wa Jamii",
    sante: "Afya",
    energie: "Nishati",
    education: "Elimu",
    route_desserte_agricole: "Barabara ya Kilimo",
    habitation: "Makazi",
    mecanisation: "Umekanizaji wa Kilimo",
    tracteur: "Trakta",
    accessoires: "Vifaa",
    autres: "Vifaa Vingine",
    environement: "Mazingira",
    protection_foret: "Ulinzi wa Misitu",
    reboisement: "Upandaji Miti",
    credit_carbone: "Mkopo wa Kaboni",
    eco_kelasi: "Eco Kelasi",
    climate: "Mabadiliko ya Tabianchi",
    projects: "Miradi",
    ongoing: "Inaendelea",
    phase1: "Awamu ya 1",
    phase2: "Awamu ya 2",
    completed: "Imekamilika",
    media: "Vyombo vya Habari",
    news: "Makala",
    gallery: "Matunzio",
    emplois_stage: "Kazi & Mafunzo",
    donation: "Msaada",
    contact: "Wasiliana",
  },
  join: {
    hero: {
      title: "Better Life Inakuhitaji!",
      subtitle: "Pamoja, tutende kwa ajili ya sayari yetu na vizazi vijavyo."
    },
    mission: "Better Life inatekeleza dhamira zilizohamasishwa kupitia masuala manne makuu: mazingira, mabadiliko ya tabianchi, bioanuwai, na usalama wa chakula.",
    intro: {
      warning: "Onyo kuhusu ongezeko la joto duniani limekuwa wazi sana kwa muda mrefu. Tunakabiliwa na mgogoro wa hali ya hewa duniani.",
      call: "Kuwa mwanachama na shiriki kikamilifu katika vitendo vyetu."
    },
    membershipType: {
      title: "Uraia",
      options: {
        lowIncome: "Mshirika wa Kipato cha Chini ($150)",
        basic: "Mshirika wa Kawaida ($300)",
        accompanying: "Mshirika Sahaba ($500)",
        supporting: "Mshirika Msaidizi ($600)",
        social: "Mshirika wa Kijamii ($1,200)",
        voluntary: "Mshirika wa Hiari / Msaada (Kiasi chochote)",
        benefactor: "Mshirika Mfadhili ($10,000+)",
        honorary: "Mwanachama wa Heshima",
      }
    },
    personalInfo: {
      civility: "Cheo",
      mr: "Bw.",
      mrs: "Bibi",
      couple: "Wanandoa",
      lastname: "Jina kamili",
      profession: "Kazi",
      nationality: "Uraia",
      status: "Hali",
      physicalPerson: "Mtu wa Kawaida",
      moralPerson: "Shirika/Taasisi",
      raison_sociale: "Jina la biashara",
      id_number: "Namba ya Kitambulisho",
      hectare: "Hekta",
      village: "Kijiji",
      benevolat_question: "Je, ungependa kuwasiliana nasi kwa ajili ya kujitolea?",
      yes: "Ndiyo",
      no: "Hapana"
    },
    addressContact: {
      title: "Anwani na Mawasiliano",
      address: "Anwani",
      code_postal: "Sanduku la Posta",
      neighborhood: "Mtaa",
      city: "Mji",
      province: "Mkoa",
      country: "Nchi",
      phone: "Simu",
      email: "Barua pepe"
    },
    paymentPreference: {
      title: "3. Njia ya Malipo",
      bank: "Uhamisho wa Benki",
      card: "Kadi",
      cash: "Pesa Taslimu",
      check: "Hundi",
      other: "Nyingine (fafanua)",
      note: "Tafadhali kumbuka kuwa ada ya uanachama hairudishwi. Mara tu uanachama wako utakapoidhinishwa, hakuna urejesho utakaofanywa."
    },
    sectors: {
      title: "4. Sekta ya Maslahi (Weka alama kwenye masanduku yanayohusika)",
      food_security: "Usalama wa Chakula",
      health_insurance: "Bima ya afya",
      large_scale_agri: "Miradi mikubwa ya kilimo",
      machinery: "Ununuzi wa mitambo ya kilimo",
      carbon_credits: "Mikopo ya kaboni",
      medicinal_plants: "Mimea ya dawa na asili",
      fundraising: "Kuchangisha fedha",
      env_protection: "Ulinzi wa mazingira",
      training_health_agri: "Mafunzo ya afya na kilimo"
    },
    contributions: {
      title: "5. Michango na Ushiriki",
      question: "Je, ungependa kutoa mchango gani?",
      financial: "Mchango wa kifedha",
      technical: "Msaada wa kiufundi / Utaalamu",
      active_agri: "Ushiriki amilifu katika miradi ya kilimo",
      other: "Nyingine"
    },
    experience: {
      title: "6. Uzoefu",
      question: "Je, una uzoefu wa awali katika miradi ya kilimo au mazingira?",
      details_label: "Ikiwa ndiyo, tafadhali toa maelezo juu ya uzoefu wako:",
      why_join_label: "Kwa nini unataka kujiunga na shirika?",
      yes: "Ndiyo",
      no: "Hapana"
    },
    cgu: {
      title: "7. Idhini na Maelekezo",
      text: "Ninathibitisha kuwa nimesoma malengo na sheria za shirika, na ninaahidi kuheshimu vigezo na masharti yaliyowekwa na mikataba yake.",
      readAndApproved: "Nimesoma na nimekubali",
      date: "Tarehe",
      signature: "Sahihi",
      signaturePlaceholder: "Andika jina lako hapa kama sahihi",
      link: "Soma masharti kamili hapa",
      submit: "Tuma Maombi",
      submitting: "Inatuma maombi..."
    },
    form: {
      title: "Fomu ya Uanachama",
      success: "Hongera! Ombi lako la uanachama limerekodiwa kwa mafanikio.",
      error: "Hitilafu imetokea wakati wa kutuma."
    }
  },
  career: {
    hero: {
      titleJob: "Omba Kazi",
      titleInternship: "Omba Mafunzo",
      subtitle: "Jiunge na timu yetu na uchangie mustakabali endelevu."
    },
    form: {
      title: "Fomu ya Maombi",
      type: "Aina ya Maombi",
      job: "Kazi",
      internship: "Mafunzo",
      cv: "Wasifu (CV)",
      cvPlaceholder: "Kiungo cha CV yako au jina la faili",
      motivation: "Barua ya Maombi / Ujumbe",
      motivationPlaceholder: "Tuambie kuhusu nia yako...",
      education: "Kiwango cha Elimu",
      educationPlaceholder: "mfano, Uzamili katika Kilimo",
      experience: "Uzoefu Unaohusika",
      experiencePlaceholder: "Muhtasari wa uzoefu wako uliopita",
      submit: "Tuma Maombi",
      success: "Maombi yako yametunwa kwa mafanikio! Tutakuwasiliana hivi karibuni."
    }
  },
  employment: {
    hero: {
      title: "Kazi & Mafunzo",
      subtitle: "Jiunge na timu yetu na uchangie mustakabali endelevu."
    },
    summary: {
      title: "Fursa Zetu za Kazi",
      text: "Katika Better Life, tunatafuta kila mara watu wenye shauku wanaotaka kuleta mabadiliko katika ulinzi wa mazingira na maendeleo endelevu nchini DRC."
    },
    jobs: {
      title: "Kazi za Kitaalamu",
      description: "Gundua nafasi zetu za kazi za sasa na ujiunge na timu ya wataalamu waliojitolea kwa mazingira.",
      btn: "Omba Kazi"
    },
    internships: {
      title: "Mafunzo kwa Wanafunzi",
      description: "Kuza ujuzi wako na upate uzoefu wa nyanjani kwa kuchangia katika miradi yetu mbalimbali.",
      btn: "Omba Mafunzo"
    }
  },
  conditions: {
    title: "Masharti ya Jumla ya Uanachama",
    backToForm: "Rudi kwenye fomu",
    sections: {
      regulation: {
        title: "1. Malengo na Ushiriki",
        text: "• Kukubaliana na wito wa shirika: Mshirika lazima azingatie malengo ya shirika: Kukuza Usalama wa Chakula, Ulinzi wa Mazingira, na Maendeleo ya Miradi ya Kilimo ya Jamii.\n• Ushiriki amilifu: Mshirika anajitolea kushiriki kikamilifu katika shughuli na miradi ya shirika, ikiwa ni pamoja na usimamizi wa miradi ya kilimo na mazingira."
      },
      types: {
        title: "2. Aina za uanachama",
        text: "• Uanachama wa mtu binafsi: Uwazi kwa watu wanaotaka kushiriki kibinafsi.\n• Uanachama wa kikundi au shirika: Uwazi kwa vyama vya ushirika vya kilimo, biashara, au vikundi vya jamii vinavyotaka kushiriki katika miradi mikubwa.\n• Uanachama wa taasisi mshirika: Kwa taasisi zinazotaka kutoa usaidizi wa kiufundi au wa kifedha."
      },
      criteria: {
        title: "3. Vigezo vya uanachama",
        text: "• Uraia na makazi: Washirika lazima wawe wakazi katika eneo la kijiografia ambapo shirika linafanya kazi, au wawe na maslahi katika shughuli za eneo hili.\n• Uzoefu wa kilimo (hiari): Kwa Washirika wanaoshiriki katika miradi ya kilimo, uzoefu katika kilimo au usimamizi wa ardhi unaweza kuhitajika.\n• Nia katika kilimo endelevu na mazingira: Washirika lazima waonyeshe nia katika kilimo endelevu, kupunguza uzalishaji wa kaboni, na uhifadhi wa mazingira."
      },
      fees: {
        title: "4. Ada za Uanachama",
        text: "• Ada ya kila mwaka: Washirika wanatakiwa kulipa ada ya uanachama, ambayo inaweza kutofautiana kulingana na aina ya uanachama wa mtu binafsi au shirika.\n• Ada za mradi: Katika baadhi ya matukio, Washirika wanaweza kualikwa kuchangia kifedha kwa miradi maalum kwa ajili ya ununuzi wa mitambo ya kilimo au utekelezaji wa programu za mazingira/kilimo."
      },
      responsibilities: {
        title: "5. Majukumu ya Mshirika",
        text: "• Ushiriki katika mikutano: Washirika wanapaswa kushiriki katika mikutano mikuu na mikutano ya kujadili miradi inayoendelea, maamuzi ya kimkakati, na usimamizi wa fedha.\n• Kuzingatia kanuni: Washirika lazima waheshimu sheria za ndani, katiba ya shirika, na kanuni maalum za mradi.\n• Mchango kwa miradi ya jamii: Washirika lazima washiriki kikamilifu katika kuchangisha fedha, mipango ya uhamasishaji, na shughuli za kilimo/mazingira."
      },
      resources: {
        title: "6. Upatikanaji wa Rasilimali na Faida",
        text: "• Upatikanaji wa mitambo ya kilimo: Washirika wana fursa ya upendeleo ya kupata vifaa na mitambo chini ya masharti maalum.\n• Upatikanaji wa mikopo ya kaboni: Washirika wanaweza kushiriki katika tathmini na programu za ufikiaji, na kugawana faida.\n• Mafunzo na usaidizi wa kiufundi: Washirika wanaweza kufaidika na mafunzo juu ya mbinu za kisasa, usimamizi wa mazingira, na teknolojia endelevu."
      },
      termination: {
        title: "7. Masharti ya Kusitisha",
        text: "• Ukiukaji wa ahadi: Mshirika anaweza kuondolewa kwa kushindwa mara kwa mara au hatua zinazokwenda kinyume na malengo.\n• Kutolipa ada: Ikiwa Mshirika hatakulipa mchango wao, anaweza kupoteza haki za uanachama."
      },
      projects: {
        title: "8. Vigezo maalum vya mradi",
        text: "• Miradi mikubwa ya kilimo: Washirika lazima wawe na ardhi au waweze kushirikiana na mashamba ya asili.\n• Kuchangisha fedha kwa miradi ya jamii: Washirika wanahimizwa kushiriki katika mipango ya ufadhili."
      },
      consent: {
        title: "9. Idhini na Maelekezo",
        text: "• Ninathibitisha kuwa nimesoma malengo na sheria za shirika, na ninaahidi kuheshimu vigezo na masharti yaliyowekwa na mikataba yake."
      }
    }
  },
  quickLinks: {
    ongoing: {
      title: "Miradi Inayoendelea",
      text: "Gundua miradi yetu 12 inayofanya kazi kubadilisha maisha nchini kote",
    },
    completed: {
      title: "Miradi Iliyokamilika",
      text: "Miradi 28 imekamilika kwa mafanikio, wanufaika wa moja kwa moja 85,000+",
    },
  },
  featured: {
    kicker: "Miradi Iliyoangaziwa",
    title: "Mipango Yetu Muhimu",
    items: [
      {
        title: "Upandaji Miti kwa Wingi Kinshasa",
        location: "Kinshasa, DRC",
        status: "Inaendelea",
        beneficiaries: "50,000",
        budget: "2.5M USD",
        trees: "500,000",
      },
      {
        title: "Kilimo Mahiri cha Tabianchi Kikwit",
        location: "Kikwit, Kwilu",
        status: "Inaendelea",
        beneficiaries: "12,000",
        budget: "1.8M USD",
        trees: "0",
      },
      {
        title: "Ulinzi wa Bioanuwai Salonga",
        location: "Hifadhi ya Salonga",
        status: "Imekamilika",
        beneficiaries: "15 jamii",
        budget: "3.2M USD",
        trees: "0",
      },
    ],
  },
  cta: {
    title: "Support Our Projects",
    text: "Your contribution can make a difference in the lives of thousands",
  },
  en_cours: {
    hero: {
      title: "Ongoing Projects",
      subtitle:
        "Discover our current initiatives for sustainable development",
    },
    stats: {
      items: [
        { number: "12", label: "Active Projects" },
        { number: "75,000+", label: "Beneficiaries" },
        { number: "8.5M USD", label: "Total Budget" },
        { number: "5", label: "Provinces Covered" },
      ],
    },
    list: [
      {
        title: "Massive Reforestation Kinshasa",
        location: "Kinshasa, DRC",
        duration: "Jan 2024 - Dec 2025",
        budget: "2.5M USD",
        beneficiaries: "50,000 people",
        description:
          "Planting 500,000 trees in the outskirts of Kinshasa to fight urban deforestation.",
        objectives: [
          "Plant 500,000 trees by end of 2025",
          "Train 1,000 youth in green jobs",
          "Create 5 community nurseries",
          "Reduce urban temperature by 2°C",
        ],
        achievements: [
          "325,000 trees already planted",
          "3 nurseries operational",
          "650 youth trained",
        ],
        phase: "Phase 2",
      },
      {
        title: "Climate-Smart Agriculture Kikwit",
        location: "Kikwit, Kwilu",
        duration: "Mar 2024 - Feb 2026",
        budget: "1.8M USD",
        beneficiaries: "3,000 farmers",
        description:
          "Climate change adaptation program for farmers in the Kikwit region.",
        objectives: [
          "Train 3,000 farmers",
          "Install 200 irrigation systems",
          "Distribute resistant seeds",
          "Increase yields by 50%",
        ],
        achievements: [
          "1,350 farmers trained",
          "90 irrigation systems installed",
          "Yields +35% on average",
        ],
        phase: "Phase 1",
      },
      {
        title: "Biodiversity Protection Salonga",
        location: "Salonga National Park",
        duration: "Jun 2023 - Jun 2026",
        budget: "3.2M USD",
        beneficiaries: "15 communities",
        description:
          "Biodiversity conservation and anti-poaching in Salonga National Park.",
        objectives: [
          "Train 50 eco-guards",
          "Install 100 camera traps",
          "Create 3 ecological corridors",
          "Reduce poaching by 80%",
        ],
        achievements: [
          "50 eco-guards operational",
          "72 cameras installed",
          "Poaching reduced by 65%",
        ],
        phase: "Phase 2",
      },
    ],
    cta: {
      title: "Support Our Projects",
      text: "Contribute to support our field actions",
    },
  },
  realises: {
    hero: {
      title: "Completed Projects",
      subtitle: "28 projects successfully completed",
    },
    impact: {
      kicker: "Our Impact",
      title: "Accumulated Results 2019-2023",
    },
    factors: {
      kicker: "Success Factors",
      title: "What Makes Us Different",
    },
    cta: {
      title: "Create New Success Together",
      text: "Your support helps multiply these successes",
    },
  },
  team: {
    hero: {
      title: "Our Team",
      subtitle:
        "Passionate experts dedicated to environmental protection and sustainable development",
    },
    leadership: {
      kicker: "Leadership",
      title: "Leadership Team",
      text: "A multidisciplinary team combining scientific expertise, field experience, and passion for the environment",
    },
    members: [
      {
        name: "Ir DEKEY MOLO Blaise",
        role: "Mratibu wa Kitaifa",
        bio: "Anahakikisha uratibu wa jumla wa shughuli za NGO katika ngazi ya kitaifa.",
        expertise: ["Uratibu", "Mkakati", "Usimamizi"],
        mail: "coordination@betterlife-ong.org"
      },
      {
        name: "Ir KILUNGU KASONGO Bernard",
        role: "Mjumbe wa Kitaifa",
        bio: "Anawakilisha shirika kwa taasisi na kusimamia wajumbe wa mikoa.",
        expertise: ["Mahusiano ya Kitaasisi", "Uongozi", "Maendeleo"],
        mail: "bernardkilungu@betterlife-ong.org"
      },
      {
        name: "Mme MPOYI NGALULA Sarah",
        role: "Mkurugenzi wa Rasilimali Watu",
        bio: "Anasimamia rasilimali watu, ajira na maendeleo ya ujuzi wa timu.",
        expertise: ["Rasilimali Watu", "Ajira", "Mafunzo"],
        mail: "rh@betterlife-ong.org"
      },
      {
        name: "Mme YANGA LUBULU Déborah",
        role: "Katibu",
        bio: "Anatoa msaada wa kiutawala na usimamizi wa nyaraka za shirika.",
        expertise: ["Utawala", "Uandaji", "Ukatibu"],
        mail: "secretariat@betterlife-ong.org"
      },
      {
        name: "Mme LUTONADIO NSIMBA Florence",
        role: "Mkurugenzi wa Fedha",
        bio: "Anasimamia usimamizi wa kifedha, kibajeti na za uhasibu wa Better Life.",
        expertise: ["Fedha", "Uhasibu", "Ukaguzi"],
        mail: "compta@betterlife-ong.org"
      },
      {
        name: "Mme META NYEMABU Ornella",
        role: "Meneja wa Mradi",
        bio: "Anahusika na upangaji, utekelezaji na ufuatiliaji wa miradi ya NGO.",
        expertise: ["Usimamizi wa Mradi", "Upangaji", "Ufuatiliaji na Tathmini"],
        mail: "gestionprojet@betterlife-ong.org"
      },
      {
        name: "Mme BAKAKUWA NGALULA Marceline",
        role: "Mkurugenzi wa Kiufundi",
        bio: "Anasimamia vipengele vya kiufundi na visayansi vya programu za mazingira na kilimo.",
        expertise: ["Utaalamu wa Kiufundi", "Kilimo", "Mazingira"],
        mail: "dt@betterlife-ong.org"
      },
      {
        name: "LUMUMBA MPUMPU Nathan",
        role: "Mkurugenzi wa Mawasiliano na Masoko",
        bio: "Anaendesha mkakati wa mawasiliano na mwonekano wa shughuli za NGO.",
        expertise: ["Mawasiliano", "Masoko", "Mahusiano ya Umma"],
        mail: "betterlifeorg@gmail.com"
      },
      {
        name: "Itaainishwa",
        role: "Katibu wa Utawala",
        bio: "Anatoa msaada wa kiutawala na usimamizi wa nyaraka kwa shirika.",
        expertise: ["Utawala", "Shirika", "Ukatibu"],
        mail: "Itaainishwa"
      },
      {
        name: "Itaainishwa",
        role: "Mkurugenzi wa Fedha",
        bio: "Anasimamia usimamizi wa fedha, bajeti na uhasibu wa Better Life.",
        expertise: ["Fedha", "Uhasibu", "Ukaguzi"],
        mail: "Itaainishwa"
      },
      {
        name: "Itaainishwa",
        role: "Mkurugenzi wa Mawasiliano na Masoko",
        bio: "Anaongoza mkakati wa mawasiliano na uonekano wa vitendo vya NGO.",
        expertise: ["Mawasiliano", "Masoko", "Mahusiano ya Umma"],
        mail: "Itaainishwa"
      },
    ],
    advisory: {
      kicker: "Advisory Board",
      title: "Our Expert Advisors",
      text: "International experts guiding our strategy and programs",
      members: [
        {
          name: "Dr. Amina Diallo",
          role: "Climate Expert",
          org: "African Climate Institute",
        },
        {
          name: "Prof. Robert Mwamba",
          role: "Biodiversity Specialist",
          org: "University of Kinshasa",
        },
        {
          name: "Sophie Laurent",
          role: "Development Consultant",
          org: "UN Environment",
        },
      ],
    },
    cta: {
      title: "Join Our Team",
      text: "We are constantly looking for talented and passionate people to strengthen our team. If you share our vision of a sustainable future, we would like to meet you.",
      items: [
        "Volunteers",
        "Interns",
        "Expert Consultants",
        "Local Partners",
      ],
      btn: "Apply Now",
    },
  },
  partners: {
    hero: {
      title: "Our Partners",
      subtitle:
        "Together, we create lasting impact for the environment and communities",
    },
    intro: {
      kicker: "Collaboration",
      title: "The Power of Partnership",
      text: "Our strategic partnerships allow us to multiply our impact and achieve our ambitious goals for environmental protection and sustainable development.",
    },
    strategic: {
      title: "International Strategic Partners",
      items: [
        {
          name: "United Nations Environment Programme",
          acronym: "UNEP",
          type: "International Organization",
          desc: "Collaboration on biodiversity conservation programs and climate change action.",
          focus: ["Climate", "Biodiversity", "Funding"],
        },
        {
          name: "World Wide Fund for Nature",
          acronym: "WWF",
          type: "International NGO",
          desc: "Strategic partnership for protecting Congo Basin forests and conserving endangered species.",
          focus: ["Conservation", "Forests", "Wildlife"],
        },
        {
          name: "African Development Bank",
          acronym: "AfDB",
          type: "Financial Institution",
          desc: "Financial and technical support for our sustainable agriculture and rural development projects.",
          focus: ["Funding", "Agriculture", "Infrastructure"],
        },
      ],
    },
    local: {
      title: "Local and National Partners",
      items: [
        {
          name: "Ministry of Environment and Sustainable Development",
          type: "DRC Government",
          collab: "Coordination of national environmental policies",
        },
        {
          name: "University of Kinshasa",
          type: "Academic Institution",
          collab: "Scientific research and training",
        },
        {
          name: "Farmers Federation of Congo",
          type: "Professional Association",
          collab: "Training and support for farmers",
        },
        {
          name: "Local Communities Network",
          type: "Community Organization",
          collab: "Mobilization and community participation",
        },
      ],
    },
    technical: {
      title: "Technical and Scientific Partners",
      items: [
        {
          name: "Agricultural Research Institute",
          expertise: "Innovative agricultural techniques",
          contrib: "Development of climate-resilient varieties",
        },
        {
          name: "Environmental Training Center",
          expertise: "Education and awareness",
          contrib: "Training programs for communities",
        },
        {
          name: "Ecological Analysis Laboratory",
          expertise: "Environmental monitoring",
          contrib: "Soil and water quality monitoring",
        },
      ],
    },
    private: {
      title: "Private Sector Partners",
      text: "We collaborate with companies committed to social and environmental responsibility:",
      items: [
        "EcoBank DRC",
        "Vodacom Foundation",
        "Rawbank",
        "Bralima",
        "Total Energies Foundation",
        "Orange DRC",
      ],
    },
    benefits: {
      kicker: "Impact",
      title: "Benefits of Our Partnerships",
      items: [
        {
          title: "Financial Resources",
          desc: "Funding for large-scale projects",
        },
        {
          title: "Technical Expertise",
          desc: "Access to global best practices",
        },
        { title: "Broader Reach", desc: "Impact in more communities" },
        { title: "Innovation", desc: "Innovative and sustainable solutions" },
      ],
    },
    cta: {
      title: "Become a Partner",
      text: "Share our vision? Let's collaborate to create positive and lasting impact",
      btn: "Contact Us",
    },
  },
  contact: {
    hero: {
      title: "Wasiliana Nasi",
      subtitle: "Usisite kuwasiliana nasi kwa swali lolote au ushirikiano",
    },
    info: {
      kicker: "Baki na Mawasiliano",
      title: "Habari zetu za Mawasiliano",
      address: {
        title: "Anwani",
        value: "N° 5 Av. Des Etangs, Q/ Joli Parc, C/ Ngaliema, Kinshasa - RDC",
      },
      email: {
        title: "Barua pepe",
        value: "infos@betterlife-ong.org",
      },
      phone: {
        title: "Simu",
        value: "+243 82 9495 919",
      },
      hours: {
        title: "Saa za kazi",
        value: "Jumatatu - Ijumaa: 8:00 - 17:00",
      },
    },

    form: {
      title: "Tutumie ujumbe",
      name: "Jina kamili",
      name_placeholder: "Jina lako",
      email: "Barua pepe",
      email_placeholder: "barua_pepe@mfano.com",
      subject: "Mada",
      subject_placeholder: "Mada ya ujumbe wako",
      message: "Ujumbe",
      message_placeholder: "Ujumbe wako...",
      btn: "Tuma ujumbe",
    },
    map: {
      title: "Tupate kwenye ramani",
      text: "N° 5 Av. Des Etangs, Q/ Joli Parc, C/ Ngaliema, Kinshasa - RDC",
    },
  },
  home: {
    hero: {
      title: "Chagua Mazingira Yaliyo Salama,\nChakula Chenye Afya",
      subtitle:
        "Dhamira yetu ni kulinda mazingira, bioanuwai, kupigania uhifadhi wa mazingira, ustawi wa watu na kuchochea usalama wa chakula.",
      cta1: "Gundua vitendo vyetu",
      cta2: "Wasiliana nasi",
      slides: [
        {
          title: "Ulinzi wa Mazingira na Bioanuwai",
          description: "Kupambana na ukataji miti na kuhifadhi mifumo ya kipekee ya DRC kwa ajili ya mustakabali endelevu."
        },
        {
          title: "Usalama wa Chakula",
          description: "Kuboresha uzalishaji wa kilimo na minyororo ya thamani ya ndani kulisha watu wetu kwa njia endelevu."
        },
        {
          title: "Umekanizaji wa Kilimo",
          description: "Kukarabati mazoea na kutoa vifaa vinavyofaa kwa kilimo cha hali ya juu."
        },
        {
          title: "Miradi ya Jamii",
          description: "Huduma za msingi na shughuli zinazozalisha mapato kwa ajili ya maendeleo ya vijiji vyetu."
        },
        {
          title: "Ufugaji wa Wanyama wa Kiwango Kikubwa",
          description: "Mifumo ya kisasa (ng'ombe, mbuzi, nguruwe, kuku) na afya ya wanyama katika moyo wa ujuzi wetu."
        },
        {
          title: "Kilimo",
          description: "Kuchochea kilimo endelevu kikuzingatia kakao ya Criollo, kahawa ya Arabica, na chai."
        }
      ]
    },
    stats: {
      title: "Athari Zetu kwa Namba",
      subtitle: "Matokeo halisi yanayobadilisha maisha",
    },
    mission: {
      kicker: "Dhamira Yetu",
      title: "Kuchukua Hatua kwa Mabadiliko ya Tabianchi na Bioanuwai",
      desc1:
        "Better Life inawasaidia watu kuelewa kwa urahisi kwa nini tabia fulani za binadamu zina athari hasi kwa bioanuwai na inapendekeza njia halisi za hatua.",
      desc2:
        "Tumeunda vituo vya miche shuleni vya miti milioni moja na laki tano ya spishi za misitu na matunda kupambana na ongezeko la joto duniani.",
      stat1_val: "3M+",
      stat1_label: "Miti iliopandwa",
      stat2_val: "2021",
      stat2_label: "Mwaka wa kuanzishwa",
      cards: {
        biodiversity: {
          title: "Bioanuwai",
          desc: "Kulinda mifumo ya ikolojia dhaifu",
        },
        agriculture: {
          title: "Kilimo",
          desc: "Mazoea mahiri ya mabadiliko ya tabianchi",
        },
        reforestation: {
          title: "Uoteshaji Miti",
          desc: "Kupambana na ongezeko la joto duniani",
        },
        community: {
          title: "Jamii",
          desc: "Miradi ya maendeleo ya ndani",
        },
      },
    },
    projects: {
      kicker: "Mafanikio Yetu",
      title: "Miradi ya Hivi Punde",
      cards: {
        p1: {
          title: "Uoteshaji Miti wa Kiwango Kikubwa Kinshasa",
          location: "Kinshasa, DRC",
          impact: "Miti 500,000 zimepandwa",
          status: "Inaendelea",
        },
        p2: {
          title: "Kilimo Endelevu Bas-Congo",
          location: "Kongo Central",
          impact: "Wakulima 1,200 wamefunzwa",
          status: "Imekamilika",
        },
        p3: {
          title: "Ulinzi wa Bioanuwai wa Salonga",
          location: "Hifadhi ya Kitaifa",
          impact: "Jamii 15 wanufaika",
          status: "Inaendelea",
        },
      },
    },
    testimonials: {
      kicker: "Ushuhuda",
      title: "Wanachosema kuhusu sisi",
      t1: {
        quote:
          "Kwa shukrani kwa Better Life, mapato yangu yamezidi mara tatu na ardhi yangu ni yenye rutuba zaidi kuliko awali. Mafunzo ya kilimo endelevu yamebadilisha maisha yangu kweli.",
        author: "Marie Nkulu",
        role: "Mkulima, Kongo Central",
      },
      t2: {
        quote:
          "Mradi huu umebadilisha shule yetu. Wanafunzi sasa ni balozi wa mazingira wanashiriki kikamilifu katika kituo cha miche cha shule.",
        author: "Mkurugenzi wa Shule ya Msingi",
        role: "Ngaliema, Kinshasa",
      },
    },
    cta: {
      title: "Jiunge na Dhamira Yetu",
      subtitle:
        "Pamoja, tunaweza kuunda mustakabali endelevu kwa DRC na kulinda mazingira yetu",
      btn1: "Changia",
      btn2: "Kuwa Mshirika",
    },
  },
  about: {
    mission: {
      hero: {
        title: "Wito Wetu",
        subtitle: "Kulinda mazingira na kukuza maendeleo endelevu kwa vizazi vijavyo",
      },
      vision: {
        kicker: "Maono Yetu",
        title: "Mustakabali Endelevu kwa Wote",
        text: "Tunawazia ulimwengu ambapo maelewano kati ya binadamu na asili yanarejeshwa, ambapo kila jamii inapata chakula bora na mazingira yaliyohifadhiwa.",
        desc: "Better Life imejitolea kuunda matokeo chanya na ya kudumu katika Jamhuri ya Kidemokrasia ya Kongo na kwingineko, kwa kuweka ulinzi wa mazingira na ustawi wa jamii katika kitovu cha hatua zetu zote.",
      },
      mission: {
        kicker: "Wito Wetu",
        title: "Kuchukua Hatua kwa Mabadiliko",
        text: "Tunafanya kazi kila siku kwa ajili ya ulinzi wa mazingira, uhifadhi wa bioanuwai na kukuza usalama wa chakula.",
        desc: "Kupitia programu bunifu na shirikishi, tunahamasisha jamii za asili kuunda mabadiliko chanya na ya kudumu yanayomnufaisha kila mmoja.",
      },
      values: {
        kicker: "Maadili Yetu",
        title: "Nguzo za Hatua Zetu",
        items: [
          {
            title: "Mikopo Midogo na Ujasiriamali",
            desc: "Usaidizi wa kifedha na kiufundi kwa ajili ya maendeleo ya shughuli zinazozalisha mapato.",
            impact: "Biashara ndogo ndogo 500+ zimesaidiwa"
          },
          {
            title: "Mafunzo ya Ufundi",
            desc: "Ukuzaji wa ujuzi wa kiufundi na ujasiriamali kwa vijana na wanawake.",
            impact: "Watu 2000+ wamefunzwa"
          },
          {
            title: "Upatikanaji wa Maji Safi",
            desc: "Ufungaji wa visima na mifumo ya usambazaji wa maji katika jamii za vijijini.",
            impact: "Vituo vya maji 50+ vimejengwa"
          },
          {
            title: "Kilimo Endelevu",
            desc: "Mafunzo ya kiikolojia ya kilimo na mbinu za kilimo mseto kwa ajili ya uzalishaji endelevu.",
            impact: "Wakulima 1000+ wamefunzwa"
          },
          {
            title: "Usalama wa Chakula",
            desc: "Programu za lishe na mseto wa chakula ili kupambana na utapiamlo.",
            impact: "Watu 5000+ wamefikiwa"
          },
          {
            title: "Uwezeshaji wa Wanawake",
            desc: "Programu maalum za kuimarisha uongozi na uhuru wa kiuchumi wa wanawake.",
            impact: "Wanawake 1000+ wamesaidiwa"
          }
        ],
      },
      impact: {
        kicker: "Maeneo ya Hatua Zetu",
        title: "Mahali Tunapofanya Kazi",
        items: [
          {
            title: "Ulinzi wa Mazingira",
            desc: "Uoteshaji miti, uhifadhi wa misitu, mapambano dhidi ya ukataji miti na uhifadhi wa mifumo ya asili.",
          },
          {
            title: "Usalama wa Chakula",
            desc: "Kukuza kilimo endelevu, mafunzo ya mbinu za kisasa za kilimo na usaidizi kwa wakulima wa asili.",
          },
          {
            title: "Bioanuwai",
            desc: "Ulinzi wa viumbe walio katika hatari, urejeshaji wa makazi ya asili na kutoa elimu juu ya umuhimu wa bioanuwai.",
          },
          {
            title: "Maendeleo ya Jamii",
            desc: "Uwezeshaji wa jamii, kuunda fursa za kiuchumi endelevu na kuboresha hali ya maisha.",
          },
        ],
      },
      cta: {
        title: "Jiunge na Wito Wetu",
        text: "Pamoja, tunaweza kuunda mustakabali wa kijani kibichi na endelevu kwa wote",
        btn_volunteer: "Kuwa Jitolea",
        btn_contact: "Wasiliana Nasi",
      },
    },
    team: {
      hero: {
        title: "Timu Yetu",
        subtitle: "Wataalamu wenye shauku waliojitolea kulinda mazingira na maendeleo endelevu",
      },
      leadership: {
        kicker: "Uongozi",
        title: "Timu ya Uongozi",
        text: "Timu ya fani mbalimbali inayochanganya ujuzi wa kisayansi, uzoefu wa nyanjani, na shauku kwa mazingira",
      },
      members: [
        {
          name: "Dr. Jean-Baptiste Mukendi",
          role: "Mkurugenzi Mkuu",
          bio: "Mtaalamu wa maendeleo endelevu mwenye uzoefu wa zaidi ya miaka 15 katika uhifadhi wa mazingira katika Afrika ya Kati. Mpenzi na ulindaji wa bioanuwai na maendeleo ya jamii za kijiji.",
          expertise: ["Uongozi", "Mkakati", "Uhifadhi"],
          mail: "delegue@betterlife.cd"
        },
        {
          name: "Marie-Claire Kabamba",
          role: "Mkurugenzi wa Operesheni",
          bio: "Mtaalamu wa usimamizi wa miradi ya mazingira na maendeleo ya jamii, mwenye shauku ya kuwezesha jamii za asili.",
          expertise: ["Usimamizi wa Miradi", "Maendeleo", "Mafunzo"],
          mail: "marie.claire@betterlife.cd"
        },
        {
          name: "Prof. André Tshiala",
          role: "Mshauri wa Kisayansi",
          bio: "Mwanabiolojia mashuhuri aliyebobea katika bioanuwai ya kitropiki na mifumo ya ikolojia ya misitu ya Bonde la Kongo.",
          expertise: ["Bioanuwai", "Utafiti", "Mifumo ya Ikolojia"],
          mail: "andre.tshiala@betterlife.cd"
        },
        {
          name: "Sylvie Mbuyi",
          role: "Meneja wa Kilimo Endelevu",
          bio: "Mtaalamu wa kilimo mwenye ujuzi wa kilimo kinachozingatia tabianchi na mbinu za kilimo hai zilizobadilishwa kwa hali ya hewa ya kitropiki.",
          expertise: ["Kilimo", "Kilimo Hai", "Ubunifu"],
          mail: "sylvie.mbuyi@betterlife.cd"
        },
        {
          name: "Patrick Nsimba",
          role: "Mratibu wa Uoteshaji Miti",
          bio: "Mhandisi wa misitu anayeratibu programu zetu za uoteshaji miti na kurejesha misitu iliyoharibiwa.",
          expertise: ["Misitu", "Uoteshaji Miti", "Vituo vya Miche"],
          mail: "patrick.nsimba@betterlife.cd"
        },
        {
          name: "Grâce Lumingu",
          role: "Meneja wa Mawasiliano",
          bio: "Mtaalamu wa mawasiliano ya mazingira na uhamasishaji jamii, anayetoa elimu juu ya changamoto za kiikolojia.",
          expertise: ["Mawasiliano", "Uhamasishaji", "Vyombo vya Habari"],
          mail: "grace.lumingu@betterlife.cd"
        },
      ],
      advisory: {
        kicker: "Bodi ya Ushauri",
        title: "Washauri Wetu Wataalamu",
        text: "Wataalamu wa kimataifa wanaoongoza mkakati na programu zetu",
        members: [
          {
            name: "Dr. Amina Diallo",
            role: "Mtaalamu wa Tabianchi",
            org: "Taasisi ya Tabianchi ya Afrika",
          },
          {
            name: "Robert Smith",
            role: "Mtaalamu wa Uhifadhi",
            org: "Mfuko wa Mazingira wa Dunia",
          },
          {
            name: "Dr. Elena Rossi",
            role: "Mtafiti wa Kilimo Hai",
            org: "Kituo cha Euro-Mediterranean",
          },
        ]
      },
      cta: {
        title: "Jiunge na Timu",
        text: "Je, unataka kutumia ujuzi wako kwa ajili ya sababu nzuri? Jiunge nasi!",
        items: [
          "Wajitolea",
          "Wanagenzi",
          "Washauri",
          "Washirika wa Ndani",
        ],
        btn: "Angalia Nafasi"
      }
    },
    partners: {
      hero: {
        title: "Washirika Wetu",
        subtitle: "Pamoja, tunatengeneza matokeo endelevu kwa mazingira na jamii",
      },
      intro: {
        kicker: "Ushirikiano",
        title: "Nguvu ya Ushirikiano",
        text: "Ushirikiano wetu wa kimkakati unatuwezesha kuzidisha matokeo yetu na kufikia malengo yetu makubwa ya ulinzi wa mazingira na maendeleo endelevu.",
      },
      strategic: {
        title: "Washirika wa Kimkakati wa Kimataifa",
        items: [
          {
            name: "Mpango wa Umoja wa Mataifa wa Mazingira",
            acronym: "UNEP",
            type: "Shirika la Kimataifa",
            desc: "Ushirikiano katika programu za uhifadhi wa bioanuwai na mapambano dhidi ya mabadiliko ya tabianchi.",
            focus: ["Tabianchi", "Bioanuwai", "Ufadhili"],
          },
          {
            name: "Mfuko wa Mazingira wa Dunia",
            acronym: "WWF",
            type: "Shirika la Kimataifa (NGO)",
            desc: "Ushirikiano wa kimkakati kwa ajili ya ulinzi wa misitu ya Bonde la Kongo na uhifadhi wa viumbe walio katika hatari.",
            focus: ["Uhifadhi", "Misitu", "Wanyamapori"],
          },
          {
            name: "Benki ya Maendeleo ya Afrika",
            acronym: "AfDB",
            type: "Taasisi ya Kifedha",
            desc: "Usaidizi wa kifedha na kiufundi kwa ajili ya miradi yetu ya kilimo endelevu na maendeleo ya vijijini.",
            focus: ["Ufadhili", "Kilimo", "Miundombinu"],
          },
        ],
      },
      local: {
        title: "Washirika wa Ndani na Kitaifa",
        items: [
          {
            name: "Wizara ya Mazingira na Asili",
            acronym: "MEN",
            type: "Wizara",
            desc: "Ushirikiano katika programu za uhifadhi wa bioanuwai na mapambano dhidi ya mabadiliko ya tabianchi.",
            focus: ["Tabianchi", "Bioanuwai", "Ufadhili"],
          },
        ],
      },
      technical: {
        title: "Washirika wa Kiufundi",
        items: [
          {
            name: "Vyuo Vikuu & Vituo vya Utafiti",
            desc: "Usaidizi wa kisayansi, tafiti za matokeo na ubunifu ili kuimarisha miradi yetu.",
          },
          {
            name: "Wataalamu & Washauri",
            desc: "Utaalamu wa nyanjani (kilimo mseto, bioanuwai, tabianchi) na kusaidia jamii.",
          },
          {
            name: "Mashirika ya Kiufundi",
            desc: "Uhamishaji wa ujuzi, mafunzo na utekelezaji wa suluhisho endelevu.",
          },
        ],
      },
      private: {
        title: "Washirika wa Sekta Binafsi",
        text: "Kampuni zilizojitolea zinachukua nafasi muhimu katika ufadhili, ubunifu na kuunda minyororo ya thamani endelevu.",
        items: [
          "Viwanda vya Kilimo",
          "Biashara Ndogo na za Kati",
          "Kampuni Chipukizi & Ubunifu",
          "Taasisi za Kifedha",
          "Kampuni zenye Matokeo Chanya",
          "Bidhaa Zinazowajibika",
        ],
      },
      benefits: {
        kicker: "Faida",
        title: "Kwa nini uwe mshirika?",
        items: [
          {
            title: "Matokeo Yanayopimika",
            desc: "Changia matokeo halisi nyanjani (uoteshaji miti, kilimo endelevu, jamii).",
          },
          {
            title: "Kuonekana & Uaminifu",
            desc: "Thibitisha ahadi yako ya kijamii (CSR) kwa washirika wako.",
          },
          {
            title: "Ubunifu wa Pamoja",
            desc: "Tutuze suluhisho linalofaa mahitaji ya nyanjani kwa pamoja.",
          },
          {
            title: "Mtandao",
            desc: "Pata ufikiaji wa mtandao wa washirika wa kiserikali, kiufundi na kijamii.",
          },
        ],
      },
      cta: {
        title: "Kuwa Mshirika",
        text: "Wasiliana nasi ili kujenga ushirikiano unaofaa malengo yako na wito wetu.",
        btn: "Wasiliana Nasi",
      },
    },
    // Moved footer to top level

    Actions: {
      environement: "Mazingira & Hali ya Hewa",
      index: {
        kicker: "Vitendo Vyetu",
        title: "Vitendo Vyetu",
        subtitle: "Vichocheo vya Athari Inayodumu",
        approach: {
          kicker: "Njia Iliyounganishwa"
        },
        objectives: {
          kicker: "Malengo Muhimu"
        },
        cards: {
          agriculture: {
            title: "Kilimo",
            desc: "Kubadilisha kilimo na kufuata mazoea mahiri kwa ajili ya usalama wa chakula."
          },
          elevage: {
            title: "Ufugaji",
            desc: "Mifumo mahiri inayopunguza uzalishaji wa gesi chafu."
          },
          communautaire: {
            title: "Maendeleo ya Jamii",
            desc: "Kukuza ustawi wa watu, afya, elimu na maendeleo shirikishi vijijini."
          },
          mecanisation: {
            title: "Umekanizaji",
            desc: "Ufumbuzi wa zana za uzalishaji kwa kilimo chenye tija zaidi."
          },
          environement: {
            title: "Mazingira",
            desc: "Vichocheo shuleni, uoteshaji miti, na kupambana na ongezeko la joto duniani."
          }
        }
      },
      agriculture: {
        hero: {
          title: "Kilimo Endelevu",
          subtitle: "Kukuza mazoea ya kilimo ambayo ni rafiki kwa mazingira na yenye faida kiuchumi",
        },
        intro: {
          kicker: "Njia Yetu",
          title: "Kilimo Mahiri",
          text: "Tunasaidia wakulima katika mabadiliko kuelekea mazoea endelevu yanayoongeza tija huku yakihifadhi mazingira.",
          subtext: "Kukabiliana na changamoto za hali ya hewa, tunahimiza kilimo chenye ustahimilivu.",
          objectives: {
            title: "Malengo Yetu",
            items: [
              "Kuongeza tija",
              "Kufundisha wakulima",
              "Kupunguza dawa za kuua wadudu",
              "Usalama wa chakula"
            ]
          }
        },
        programs: {
          kicker: "Programu Zetu",
          title: "Mipango ya Kilimo",
          items: [
            {
              title: "Kilimo mseto",
              desc: "Ushirikiano wa miti katika mazao ili kurejesha udongo na kubadilisha mapato.",
              benefits: ["Ulinzi wa udongo", "Utofauti", "Bioanuwai"]
            },
            {
              title: "Mazao ya Kudumu",
              desc: "Maendeleo ya mazao endelevu kama kakao na kahawa chini ya kivuli.",
              benefits: ["Mapato thabiti", "Kivuli cha asili", "Ubora wa juu"]
            },
            {
              title: "Usimamizi wa Maji",
              desc: "Mbinu bora za umwagiliaji na uhifadhi wa maji.",
              benefits: ["Uchumi", "Ustahimilivu", "Uzalishaji wa mwaka mzima"]
            },
            {
              title: "Mbegu Zilizoboreshwa",
              desc: "Usambazaji wa mbegu zinazostahimili na kurekebishwa kulingana na mabadiliko ya tabianchi.",
              benefits: ["Mavuno bora", "Upinzani dhidi ya magonjwa", "Marekebisho"]
            },
            {
              title: "Urejelezaji & Mboji",
              desc: "Ubadilishaji wa taka za kilimo kuwa mbolea ya asili ya ubora wa juu.",
              benefits: ["Rutuba ya asili", "Taka sifuri", "Uchumi wa pembejeo"]
            },
            {
              title: "Uhifadhi wa Udongo",
              desc: "Mazoea ya kupambana na mmomonyoko wa udongo na kuboresha mfumo wa udongo.",
              benefits: ["Uendelevu", "Uhifadhi wa maji", "Mimea ya udongo"]
            }
          ]
        },
        training: {
          title: "Mafunzo na Usaidizi",
          items: [
            {
              title: "Mbinu za Kilimo",
              desc: "Kujifunza mbinu za kilimo hai na endelevu."
            },
            {
              title: "Usimamizi wa Wadudu",
              desc: "Mapambano ya kibaolojia yaliyojumuishwa dhidi ya magonjwa na wadudu waharibifu."
            },
            {
              title: "Uongezaji Thamani",
              desc: "Uboreshaji wa bidhaa za kilimo ili kuongeza mapato."
            },
            {
              title: "Usimamizi Rahisi",
              desc: "Mafunzo juu ya usimamizi wa kimsingi wa shamba."
            }
          ],
          impact: {
            title: "Athari Yetu",
            stats: [
              { number: "1200+", label: "Wakulima waliofunzwa" },
              { number: "+40%", label: "Wastani wa mavuno" },
              { number: "15", label: "Vyama vya ushirika" },
              { number: "-85%", label: "Dawa za kuua wadudu" }
            ]
          },
          testimonial: {
            text: "Shukrani kwa mafunzo ya mseto wa misitu na kilimo, sijafanikiwa tu kuongeza uzalishaji wangu wa kakao, bali pia nimeimarisha ardhi yangu kwa ajili ya watoto wangu.",
            author: "Jean K., Mkulima mshirika"
          }
        },
        cta: {
          title: "Jiunge na harakati",
          text: "Wakulima au wapenzi, tunakuhitaji",
          btn_train: "Mafunzo",
          btn_more: "Soma Zaidi"
        }
      },
      agricultures: {
        cacao: {
          hero: { title: "Kakao (Criollo)", subtitle: "Dhahabu ya RDC" },
          intro: { kicker: "Asili", title: "Kakao ya Criollo", text: "Aina adimu na inayothaminiwa kwa harufu yake nzuri na ya hila." },
          features: { kicker: "Sifa", title: "Kwa nini Criollo?", items: ["Harufu ya matunda", "Maharagwe meupe", "Uchachushaji mfupi", "Thamani ya juu"] },
          technique: { title: "Mbinu ya Kilimo", items: ["Chini ya kivuli cha misitu", "Ushirikiano na migomba", "Uvunaji wa mwongozo uliochaguliwa"] },
          impact: { title: "Athari", stats: [{ value: "500T", label: "Uzalishaji" }, { value: "1200", label: "Wazalishaji" }] }
        },
        cafe: {
          hero: { title: "Kahawa ya Arabica", subtitle: "Ubora wa Nyanda za Juu" },
          intro: { kicker: "Terroir", title: "Kahawa ya Milimani", text: "Inapandwa kwenye mwinuko kwa utajiri wa kipekee wa harufu." },
          features: { kicker: "Ubora", title: "Kahawa Yetu", items: ["Mwinuko > 1200m", "Usindikaji uliosafishwa", "Kukausha kwenye vitanda vya Kiafrika", "Alama ya SCA 85+"] },
          technique: { title: "Mchakato", items: ["Uchumaji mzuri", "Kuondoa mimbari siku hiyo hiyo", "Uchachushaji unaodhibitiwa"] },
          impact: { title: "Athari", stats: [{ value: "300T", label: "Mauzo ya nje" }, { value: "800", label: "Familia" }] }
        },
        the: {
          hero: { title: "Chai", subtitle: "Utamaduni wa Ustawi" },
          intro: { kicker: "Mila", title: "Chai ya Virunga", text: "Wakulima wenye shauku kwa chai yenye tabia dhabiti." },
          features: { kicker: "Bidhaa", title: "Aina za Chai", items: ["Chai nyeusi ya kiorthodox", "Chai ya kijani", "Chai nyeupe (machipukizi)", "Iliyothibitishwa kuwa Bio"] },
          technique: { title: "Ujuzi", items: ["Uchumaji wa kifalme (chipukizi moja, majani mawili)", "Kunyauka kiasili", "Kuviringisha kwa mikono"] },
          impact: { title: "Athari", stats: [{ value: "150T", label: "Uzalishaji" }, { value: "450", label: "Wanawake walioajiriwa" }] }
        },
        coton_caoutchouc: {
          hero: { title: "Pamba na Mpira", subtitle: "Mazao ya Biashara ya Viwandani" },
          intro: { kicker: "Viwanda", title: "Sekta za Kimkakati", text: "Ufufuaji wa mazao ya viwandani kwa uchumi wa ndani." },
          features: { kicker: "Sekta", title: "Athari Mbili", items: ["Pamba ya nyuzi ndefu", "Mpira wa kloni", "Usindikaji wa ndani", "Nguo na Viwanda"] },
          technique: { title: "Kilimo-viwanda", items: ["Mashamba endelevu", "Usimamizi wa kiufundi", "Umekanizaji uliorekebishwa"] },
          impact: { title: "Athari", stats: [{ value: "1000ha", label: "Eneo" }, { value: "2000", label: "Kazi za moja kwa moja" }] }
        },
        arboriculture: {
          hero: { title: "Arboriculture", subtitle: "Mashamba ya Matunda Yaliyotofautishwa" },
          intro: { kicker: "Utofauti", title: "Matunda ya Kitropiki", text: "Wingi wa ladha kwa matumizi ya ndani na nje." },
          features: { kicker: "Aina", title: "Matunda Yetu", items: ["Miembe iliyounganishwa", "Parachichi za Hass", "Machungwa", "Tunda la mateso"] },
          technique: { title: "Mazoea", items: ["Kuunganisha", "Kupogoa kwa mafunzo", "Udhibiti jumuishi", "Umwagiliaji wa matone"] },
          impact: { title: "Athari", stats: [{ value: "50k", label: "Miti" }, { value: "Vitamini", label: "Lishe" }] }
        },
        plantes_medicinales: {
          hero: { title: "Mimea ya Dawa", subtitle: "Pharmacopoeia ya Asili" },
          intro: { kicker: "Afya", title: "Maarifa ya Mababa na Sayansi", text: "Uthamini wa mimea ya dawa ya kienyeji kwa afya ya jamii." },
          features: { kicker: "Spishi", title: "Mimea Muhimu", items: ["Artemisia (Malaria)", "Moringa (Lishe)", "Prunus Africana", "Mchaichai"] },
          technique: { title: "Usindikaji", items: ["Kukausha kwa jua", "Uchituzi wa mafuta muhimu", "Ufungaji wa usafi"] },
          impact: { title: "Athari", stats: [{ value: "20", label: "Bustani za afya" }, { value: "10k", label: "Wagonjwa waliosaidiwa" }] }
        }
      },
      environement: {
        hero: {
          title: "Ulinzi wa Mazingira",
          subtitle: "Kulinda mazingira yetu na rasilimali zetu za asili"
        },
        protection_foret: {
          hero: {
            title: "Ulinzi wa Misitu",
            subtitle: "Kuhifadhi mapafu ya dunia yetu"
          },
          intro: {
            title: "Uhifadhi wa Misitu",
            text: "Mipango yetu ya ulinzi wa misitu inalenga kuhifadhi misitu yetu ya asili na kudumisha mazingira salama kwa wote.",
            highlights: [
              "Ulinzi wa wanyamapori",
              "Urejeshaji wa misitu",
              "Ufuatiliaji wa misitu",
              "Ushiriki wa jamii"
            ]
          }
        },
        intro: {
          kicker: "Ahadi Yetu",
          title: "Kutenda kwa Ajili ya Bioanuwai",
          text: "Tunatekeleza vitendo halisi vya uhifadhi wa mifumo ikolojia, usimamizi endelevu kwa rasilimali za asili, na kupambana na mabadiliko ya tabia nchi.",
          stats: {
            planted: { number: "450k", label: "Miti" },
            survival: { number: "85%", label: "Kiwango cha Ustawi" }
          }
        },
        programs: {
          kicker: "Mipango Yetu",
          title: "Mipango Yetu ya Kijani",
          items: [
            { title: "Usimamizi wa Taka", desc: "Kuchakata tena na kuongeza thamani ya taka.", benefits: ["Usafi", "Uchumi wa mzunguko"] },
            { title: "Nishati Safi", desc: "Kuhamasisha matumizi ya jiko bora.", benefits: ["Chini ya CO2", "Afya"] },
            { title: "Elimu", desc: "Sensitisheni shuleni.", benefits: ["Kizazi cha baadaye", "Ufahamu"] }
          ]
        },
        impact: {
          title: "Athari Yetu ya Kimazingira",
          stats: [
            { number: "-30%", label: "Uzalishaji wa CO2" },
            { number: "50T", label: "Taka zilizochakatwa" },
            { number: "10k", label: "Watoto waliosensitiwa" }
          ]
        },
        cta: {
          title: "Tunze Sayari Yetu",
          text: "Jiunge nasi katika dhamira yetu ya kuhifadhi asili na kujenga mustakabali endelevu.",
          btn_join: "Kuwa mwanachama",
          btn_sponsor: 'Tunza mti'
        },
        // Ukurasa wa chini
        protection_foret_section: {
          hero: {
            title: "Ulinzi wa Msitu",
            subtitle: "Mapafu ya sayari"
          },
          intro: {
            title: "Uhifadhi",
            text: "Kupambana na ukataji miti na uwindaji haramu."
          },
          features: ["Ulinzi wa walezi wa mazingira", "Ramani ya ushiriki", "Urejeshaji", "Maeneo yaliyohifadhiwa"]
        },
        credit_carbone: {
          hero: {
            title: "Stakabadhi ya Kaboni",
            subtitle: "Kuongeza thamani ya kufyonza gesi chafu"
          },
          intro: {
            title: "Fedha za Hali ya Hewa",
            text: "Pata mapato kupitia uhifadhi wa misitu."
          },
          features: ["Uthibitisho", "Uuzaji wa stakabadhi", "Mapato ya jamii", "Ufuatiliaji wa kaboni"]
        },
        eco_kelasi: {
          hero: {
            title: "Eko-Shule",
            subtitle: "Shule ya kimazingira"
          },
          intro: {
            title: "Elimu ya Mazingira",
            text: "Kujifunza kupenda na kulinda asili tangu utotoni."
          },
          features: ["Bustani za shule", "Vikundi vya asili", "Uchambuzi wa taka", "Safari za kielimu"]
        }
      },

      mecanisation: {
        hero: { title: "Umekanizaji wa Kilimo", subtitle: "Kisasa kwa ajili ya kilimo chenye tija" },
        intro: {
          kicker: "Maono Yetu",
          title: "Mustakabali ni Umekanizaji",
          text: "Tunarahisisha ufikiaji wa vifaa vya kisasa ili kuongeza tija huku tukipunguza ugumu wa kazi ya kilimo.",
          stats: [
            { value: "200+", label: "Matrekta yaliyotumwa" },
            { value: "1000+", label: "Hekta zilizolimwa" },
            { value: "50+", label: "Warsha za kutembea" },
            { value: "500+", label: "Waendeshaji waliofunzwa" }
          ]
        },
        programs: {
          kicker: "Programu Zetu",
          title: "Suluhisho za Umekanizaji",
          items: [
            {
              title: "Matrekta ya Kilimo",
              desc: "Kukodisha na kuuza matrekta yaliyorekebishwa kwa mashamba ya ndani.",
              impact: "100+ mashamba"
            },
            {
              title: "Vitengo vya Usindikaji",
              desc: "Vifaa vya usindikaji wa bidhaa za kilimo (vinu, mashine za kukamua).",
              impact: "50+ vitengo"
            },
            {
              title: "Matengenezo na Ukarabati",
              desc: "Huduma na ukarabati ili kuhakikisha maisha marefu ya vifaa.",
              impact: "200+ hatua"
            },
            {
              title: "Mafunzo ya Waendeshaji",
              desc: "Mafunzo juu ya matumizi na matengenezo ya vifaa vya kilimo.",
              impact: "500+ waendeshaji"
            }
          ]
        },
        benefits: { title: "Faida", items: ["Kuongezeka kwa tija", "Kupunguza kazi ngumu", "Uvutiaji wa vijana", "Usahihi"] },
        cta: {
          title: "Gesi Shamba Lako",
          text: "Wekeza katika umekanizaji ili kuboresha tija na faida yako.",
          btn_contact: "Wasiliana nasi",
          btn_more: "Soma zaidi"
        },
        // Sub-pages
        tracteur: {
          hero: { title: "Matrekta Yetu", subtitle: "Nguvu na uaminifu katika huduma ya mashamba" },
          intro: { title: "Meli ya Kisasa", text: "Vifaa vilivyorekebishwa kwa maeneo yetu." },
          features: ["Matrekta ya Kila Mahali", "Matumizi ya Chini", "Vifaa vinavyopatikana", "GPS iliyojumuishwa"]
        },
        accessoires: {
          hero: { title: "Vifaa vya Kilimo", subtitle: "Chombo sahihi kwa kila kazi" },
          intro: { title: "Vifaa", text: "Wapanda mbegu, jembe, na trela ili kuboresha kila hatua." },
          features: ["Jembe la diski", "Wapanda mbegu wa usahihi", "Trela za 5T", "Mifumo ya umwagiliaji"]
        },
        autres: {
          hero: { title: "Vifaa Vingine", subtitle: "Ubunifu unaoendelea" },
          intro: { title: "Utofauti", text: "Droni, vitambuzi, na vifaa vidogo vya injini." },
          features: ["Droni za ufuatiliaji", "Vitambuzi vya unyevu", "Power tillers", "Chippers"]
        }
      },
      elevage: {
        hero: {
          title: "Ufugaji Endelevu",
          subtitle: "Mazoea ya heshima na yenye faida"
        },
        intro: {
          kicker: "Njia Yetu",
          title: "Ufugaji wa Kisasa na Uwajibikaji",
          text: "Tunatengeneza mbinu za ufugaji zinazochanganya tija, ustawi wa wanyama, na heshima kwa mazingira kwa ajili ya usalama wa chakula endelevu.",
          stats: [
            { value: "5000+", label: "Mifugo" },
            { value: "15+", label: "Spishi zinazofugwa" },
            { value: "3", label: "Vituo vya mifugo" },
            { value: "100+", label: "Wafugaji waliofunzwa" }
          ]
        },
        types: {
          kicker: "Sekta Zetu",
          title: "Aina za Ufugaji Zinazoungwa Mkono",
          items: [
            {
              title: "Ufugaji wa Mbwa",
              desc: "Ufugaji wa kitaalamu wa mbwa wa asili na marafiki kwa ajili ya usalama na usaidizi.",
              production: "Wanyama wa ndani & Usalama"
            },
            {
              title: "Ufugaji wa Nyoka",
              desc: "Mpango wa utafiti na uhifadhi kwa spishi za nyoka wa ndani.",
              production: "Utafiti & Uhifadhi"
            },
            {
              title: "Mifugo Mikubwa",
              desc: "Ufugaji wa kisasa wa ng'ombe unaozingatia ubora wa nyama na uzalishaji wa maziwa.",
              production: "Maziwa & Nyama"
            },
            {
              title: "Ufugaji Nyuki",
              desc: "Uzalishaji wa asali ya asili na uhifadhi wa nyuki wa ndani.",
              production: "Asali & Bidhaa zinazotokana nayo"
            },
            {
              title: "Ufugaji wa Samaki",
              desc: "Uzalishaji endelevu wa samaki wa maji baridi katika mabwawa yaliyosimamiwa.",
              production: "Samaki wabichi & Vifaranga"
            }
          ]
        },
        actions: {
          kicker: "Huduma",
          title: "Usaidizi kwa Wafugaji",
          items: [
            { title: "Afya ya Wanyama", features: ["Chanjo", "Huduma za mifugo", "Kuzuia magonjwa"] },
            { title: "Uboreshaji wa Kinasaba", features: ["Uvukishaji", "Uteuzi", "Mifugo yenye utendaji wa juu"] },
            { title: "Mafunzo", features: ["Usimamizi wa mifugo", "Usafi", "Lishe"] },
            { title: "Miundombinu", features: ["Ujenzi wa makazi", "Mabirika ya kulia", "Mabirika ya kunywa"] }
          ]
        },
        story: {
          tag: "Ushuhuda",
          title: "Mafanikio ya Ndani",
          text: "Ufungaji wa vitengo vya kisasa vya kuku uliruhusu wanawake 50 kutoka chama cha ushirika kuongeza mapato yao mara mbili kwa mwaka mmoja.",
          benefits: ["Uhuru wa kifedha", "Usalama wa lishe", "Kujenga uwezo"],
          stats: [
            { label: "Kuku", value: "5000+", color: "green" },
            { label: "Mapato", value: "+100%", color: "blue" }
          ]
        },
        stats: {
          title: "Takwimu Muhimu",
          items: [
            { number: "250", label: "Wafugaji wanaoungwa mkono" },
            { number: "10k", label: "Mifugo" },
            { number: "5T", label: "Asali inayozalishwa" },
            { number: "12", label: "Vitengo vya samaki" }
          ]
        },
        cta: {
          title: "Jiunge na Programu Yetu",
          text: "Endeleza shughuli yako ya ufugaji kwa usaidizi wetu wa kibinafsi",
          btn_join: "Kuwa mfugaji mshirika",
          btn_partner: "Kuwa mshirika"
        },
        // Sub-pages
        gros_betail: {
          hero: { title: "Ufugaji wa Mifugo Mikubwa", subtitle: "Maendeleo ya sekta ya ng'ombe katika RDC" },
          intro: {
            kicker: "Utaalamu Wetu wa Ng'ombe",
            title: "Ufugaji wa Kisasa na Wenye Tija",
            text: "Tunasaidia wafugaji katika uboreshaji wa kinasaba wa mifugo yao, usimamizi wa malisho, na uboreshaji wa uzalishaji wa maziwa na nyama.",
            highlights: ["Uteuzi wa mifugo inayofaa hali ya hewa ya ndani", "Mbinu endelevu za ufugaji", "Ufikiaji wa huduma za mifugo", "Masoko ya bidhaa"]
          },
          features: [
            { title: "Uboreshaji wa Kinasaba", desc: "Uteuzi na uzalishaji wa mifugo ya ng'ombe yenye uwezo mkubwa wa uzalishaji." },
            { title: "Afya ya Wanyama", desc: "Programu kamili ya chanjo na huduma za kuzuia mifugo." },
            { title: "Lishe Bora", desc: "Uundaji wa chakula kinachofaa mahitaji ya lishe." },
            { title: "Usimamizi wa Malisho", desc: "Mbinu za mzunguko na urejeshaji wa malisho." }
          ],
          stats: [
            { value: "2x", label: "Kuongezeka kwa uzalishaji wa maziwa" },
            { value: "-30%", label: "Kupunguza vifo vya wanyama" },
            { value: "50+", label: "Wafugaji waliofunzwa" },
            { value: "100%", label: "Wanyama waliopewa chanjo" }
          ]
        },
        apiculture: {
          hero: { title: "Ufugaji Nyuki Endelevu", subtitle: "Dhahabu ya maji ya misitu yetu" },
          intro: {
            kicker: "Njia Yetu ya Ufugaji Nyuki",
            title: "Ulinzi wa Nyuki, Utajiri wa Jamii",
            text: "Programu yetu endelevu ya ufugaji nyuki inachanganya uhifadhi wa bioanuwai na uundaji wa mapato kwa jamii za mahali hapo.",
            highlights: ["Mizinga ya jadi iliyoboreshwa", "Mbinu endelevu za uvunaji", "Usindikaji wa bidhaa za mzinga", "Masoko ya ndani na nje"]
          },
          features: [
            { title: "Mizinga ya Kisasa", desc: "Ubunifu wa mizinga inayofaa hali ya hewa ya ndani na yenye heshima kwa nyuki." },
            { title: "Asali ya Bio Iliyothibitishwa", desc: "Uzalishaji wa asali 100% ya asili bila kemikali." },
            { title: "Nta", desc: "Uhamishaji wa nta kwa ajili ya vipodozi na bidhaa za sanaa." },
            { title: "Uchavushaji", desc: "Mchango katika uhifadhi wa mifumo ikolojia na kuongeza mavuno ya kilimo." }
          ]
        },
        pisciculture: {
          hero: { title: "Ufugaji Samaki Endelevu", subtitle: "Ufugaji wa samaki unaowajibika" },
          intro: {
            kicker: "Utaalamu Wetu wa Ukulima wa Majini",
            title: "Uvuvi Endelevu kwa Kesho",
            text: "Maendeleo ya ukulima wa majini wa nchi kavu kwa mbinu zenye heshima kwa mazingira na rasilimali za maji.",
            highlights: ["Ufugaji wa Tilapia na Clarias", "Mifumo ya mabwawa na matenki", "Lishe iliyoboreshwa", "Usimamizi endelevu wa maji"]
          },
          features: [
            { title: "Vifaranga vya Ubora", desc: "Uteuzi mkali wa wazazi na uzalishaji unaodhibitiwa." },
            { title: "Lishe Iliyoboreshwa", desc: "Inayotajiriwa na protini kwa ukuaji wa haraka na afya." },
            { title: "Usimamizi wa Maji", desc: "Mifumo ya uchujaji na urejeshaji kwa maji ya ubora." },
            { title: "Mafunzo Yanayoendelea", desc: "Usaidizi wa kiufundi kwa wakulima wa samaki kwa uzalishaji bora." }
          ]
        },
        chiens: {
          hero: { title: "Ufugaji wa Mbwa wa Kitaalamu", subtitle: "Usalama, Urafiki na Uokoaji" },
          intro: {
            kicker: "Ahadi Yetu ya Mbwa",
            title: "Mbwa Walioelimika kwa Mahitaji Yote",
            text: "Ufugaji wa mbwa wa asili kwa kuzingatia sana ustawi wa wanyama na ujuzi maalum kwa kila asili.",
            highlights: ["Shepherds wa Kijerumani mna wa Ubelgiji", "Mbwa wa ulinzi na ulinzi", "Mbwa wa usaidizi", "Wanyama wa nyumbani"]
          },
          features: [
            { title: "Ufugaji wa Kimaadili", desc: "Hali bora za maisha na heshima kwa ustawi wa wanyama." },
            { title: "Pedigree Iliyothibitishwa", desc: "Wanyama walioandikishwa na LOF na nasaba iliyodhibitiwa." },
            { title: "Elimu ya Mapema", desc: "Uhusiano wa kijamii na misingi ya elimu tangu umri mdogo." },
            { title: "Ufuatiliaji wa Mifugo", desc: "Chanjo za sasa na ripoti kamili ya afya." }
          ]
        },
        serpents: {
          hero: { title: "Kituo cha Ufugaji wa Nyoka", subtitle: "Utafiti na Uzalishaji wa Dawa za Sumu" },
          intro: {
            kicker: "Dhamira Yetu ya Kisayansi",
            title: "Uhifadhi na Utafiti juu ya Nyoka",
            text: "Kituo chetu maalum kinachangia utafiti juu ya sumu na uzalishaji wa seramu za kuzuia sumu ili kuokoa maisha.",
            highlights: ["Ufugaji uliodhibitiwa wa spishi za sumu", "Uchimbaji salama wa sumu", "Utafiti wa matibabu", "Programu za uhamasishaji"]
          },
          features: [
            { title: "Uchimbaji Salama", desc: "Itifaki kali kwa ushughulikiaji and ufuatiliaji." },
            { title: "Utafiti wa Kisayansi", desc: "Utafiti wa sifa za matibabu ya sumu." },
            { title: "Uhifadhi", desc: "Programu za ulinzi wa spishi zilizo hatarini." },
            { title: "Mafunzo", desc: "Warsha juu ya kuzuia na huduma ya kwanza ikiwa kuna kuumwa." }
          ]
        }
      },
      communautaire: {
        hero: {
          title: "Maendeleo ya Jamii",
          subtitle: "Kuwezesha jamii kwa mustakabali endelevu"
        },
        intro: {
          kicker: "Njia Yetu",
          title: "Katika moyo wa jamii za mahali hapo",
          text: "Tunafanya kazi bega kwa bega na jamii kukuza suluhisho endelevu zinazoboresha hali ya maisha huku zikihifadhi mazingira.",
          highlights: [
            "Njia ya ushiriki",
            "Maendeleo ya mtaa yaliyounganishwa",
            "Kuimarisha uwezo",
            "Kuwezesha wanawake na vijana"
          ],
          stats: [
            { value: "50+", label: "Jamii zinazoungwa mkono" },
            { value: "10k+", label: "Wanufaika wa moja kwa moja" },
            { value: "200+", label: "Miradi iliyokamilika" },
            { value: "5", label: "Mikoa iliyofikiwa" }
          ]
        },
        programs: {
          kicker: "Programu Zetu",
          title: "Mipango ya Ndani",
          items: [
            {
              title: "Mikopo Midogo na Ukuzaji Biashara",
              desc: "Usaidizi wa kifedha na kiufundi kwa ajili ya kuunda shughuli zinazozalisha mapato.",
              impact: "500+ biashara ndogo zinazoungwa mkono"
            },
            {
              title: "Elimu na Mafunzo",
              desc: "Ujenzi wa shule, mafunzo ya walimu na ufadhili wa masomo kwa watoto maskini.",
              impact: "2000+ wanafunzi wanufaika"
            },
            {
              title: "Afya ya Jamii",
              desc: "Kliniki zinazotembea, kampeni za chanjo na uhamasishaji juu ya usafi wa kimsingi.",
              impact: "10 000+ wagonjwa waliotibiwa"
            },
            {
              title: "Ufikiaji wa Maji Safi",
              desc: "Uchoraji wa visima na mifumo ya usafi kwa jamii za vijijini.",
              impact: "15 000+ watu waliopata huduma"
            },
            {
              title: "Usalama wa Chakula",
              desc: "Programu za lishe na mseto wa chakula kupambana na utapiamlo.",
              impact: "5000+ familia zinazoungwa mkono"
            },
            {
              title: "Kuwezesha Wanawake",
              desc: "Mafunzo ya ufundi na msaada kwa mipango ya biashara ya wanawake.",
              impact: "2000+ wanawake waliofunzwa"
            }
          ]
        },
        women: {
          title: "Kuwezesha Wanawake",
          text: "Tunaamini kuwa kuwezesha wanawake ndio ufunguo wa maendeleo endelevu ya jamii. Programu zetu zinalenga kuimarisha uongozi wao, ujuzi na uhuru wa kiuchumi.",
          items: [
            "Mafunzo katika usimamizi wa biashara",
            "Kujifunza kusoma na kuandika kwa watu wazima",
            "Kilimo cha mboga cha wanawake",
            "Vyama vya ushirika vya kuweka akiba na mikopo"
          ],
          stats: [
            { number: "2,000+", label: "Wanawake waliofunzwa" },
            { number: "300+", label: "Biashara ndogo zilizoundwa" },
            { number: "85%", label: "Kiwango cha mafanikio" },
            { number: "10", label: "Vyama vya ushirika vinavyoungwa mkono" }
          ],
          testimonial: {
            text: "Shukrani kwa mafunzo ya ujasiriamali, nimeweza kuanzisha biashara yangu ndogo na kukidhi mahitaji ya familia yangu.",
            author: "Marie K., Mnufaika wa programu"
          }
        },
        stories: {
          kicker: "Hadithi za Mafanikio",
          title: "Mafanikio Yetu",
          items: [
            {
              title: "Shule ya matumaini huko Kibumba",
              text: "Ujenzi na vifaa vya shule ya msingi kwa watoto 500 katika kijiji cha mbali cha Kivu Kaskazini.",
              impact: "500 watoto shuleni, walimu 15 waliofunzwa, maktaba 1 yenye vifaa"
            },
            {
              title: "Maji, chemchemi ya uzima huko Tshikapa",
              text: "Ufungaji wa visima 5 vyenye pampu, vinavyohudumia watu 3,000.",
              impact: "3,000 wanufaika, vituo 5 vya maji vilivyowekwa, kamati 10 zilizofunzwa matengenezo"
            },
            {
              title: "Wanawake wajasiriamali wa Goma",
              text: "Programu ya mafunzo na mikopo midogo kwa wanawake wajasiriamali 200 katika sekta isiyo rasmi.",
              impact: "Wanawake 200 waliofunzwa, biashara ndogo 150 zilizinduliwa, 80% kiwango cha kurejesha"
            }
          ]
        },
        cta: { title: "Shiriki", text: "Tusaidie kujenga", btn_donate: "Toa msaada", btn_volunteer: "Ujitoleaji" },
        // Sub-pages
        sante: {
          hero: { title: "Afya ya Jamii", subtitle: "Ufikiaji wa huduma kwa wote" },
          intro: { title: "Huduma za Karibu", text: "Kupambana na magonjwa na huduma za mama." },
          features: ["Kampeni za chanjo", "Kliniki zinazotembea", "Afya ya mama", "Kupambana na utapiamlo"]
        },
        energie: {
          hero: { title: "Nishati Jadidifu", subtitle: "Mwanga na Nguvu" },
          intro: { title: "Umeme Vijijini", text: "Mabwawa madogo na paneli za jua." },
          features: ["Jua la nyumbani", "Umeme mdogo wa maji", "Biogas", "Matengenezo ya ndani"]
        },
        education: {
          hero: { title: "Elimu", subtitle: "Mustakabali wa watoto wetu" },
          intro: { title: "Maarifa kwa Wote", text: "Ujenzi wa shule na ufadhili wa masomo." },
          features: ["Miundombinu ya shule", "Vifaa", "Mafunzo ya walimu", "Kujifunza kusoma na kuandika"]
        },
        routes_desserte: {
          hero: { title: "Barabara za Kilimo", subtitle: "Kufungua maeneo ya vijijini" },
          intro: { title: "Miundombinu", text: "Ukarabati wa nyimbo kwa mtiririko wa bidhaa." },
          features: ["Cantonage ya mwongozo", "Madaraja", "Matengenezo endelevu", "Ufikiaji wa masoko"]
        },
        habitation: {
          hero: { title: "Makazi ya Kijamii", subtitle: "Paa la heshima" },
          intro: { title: "Nyumba Endelevu", text: "Ujenzi wa nyumba kwa kutumia vifaa vya ndani vilivyoboreshwa." },
          features: ["Matofali yaliyoimarishwa", "Paa za kudumu", "Usafi", "Mipango ya miji"]
        }
      }
    },
    blog: {
      index: {
        hero: {
          kicker: "Habari / Blogu",
          title: "Makala na Matangazo",
          subtitle: "Pata habari kuhusu matendo yetu na habari mpya za nyanjani.",
        },
        items: {
          excerpt: "Ongeza dondoo, vyombo vya habari (picha/video), na kiungo cha makala kamili hapa.",
          readMore: "Soma Makala",
        },
        sections: {
          interviews: {
            title: "Mahojiano na Video",
            desc: "Pata rasilimali za multimedia hapa: ushuhuda wa wazalishaji, washirika, nyuma ya pazia.",
          },
          campaigns: {
            title: "Kampeni Zinazoendelea",
            desc: "Matangazo ya matukio, uchangishaji fedha, siku za upandaji miti, vikao vya mafunzo ya wazi.",
          },
        },
      },
      news: {
        hero: {
          title: "Habari",
          subtitle: "Pata habari kuhusu hatua na mafanikio yetu ya hivi punde",
        },
        intro: {
          kicker: "Habari za Hivi Punde",
          title: "Fuata Matukio Yetu",
          text: "Fuata matukio yetu, mafanikio, na miradi mipya ya maendeleo endelevu nchini DRC.",
        },
        categories: {
          title: "Vitengo",
          all: "Zote",
          announcement: "Tangazo",
          partnership: "Ushirikiano",
          training: "Mafunzo",
          success: "Mafanikio",
          infrastructure: "Miundombinu",
          recognition: "Utambuzi",
          projects: "Miradi",
          events: "Matukio",
          partnerships: "Mashirikiano",
          trainings: "Mafunzo",
        },
        filters: {
          title: "Chuja",
          date: "Tarehe",
          category: "Kitengo",
          search: "Tafuta habari...",
        },
        featured: {
          label: "Iliyoangaziwa",
        },
        readMore: "Soma zaidi →",
        empty: {
          title: "Hakuna Habari",
          text: "Hakuna makala yanayopatikana kwa wakati huu. Tafadhali rudia baadaye kwa sasisho mpya.",
        },
        newsletter: {
          title: "Pata Habari",
          text: "Jisajili kwenye jarida letu ili kupokea habari zetu moja kwa moja kwenye kisanduku chako cha barua",
          placeholder: "Anwani yako ya barua pepe",
          subscribe: "Jiunge",
        },
        articles: {
          reforestation2025: {
            title: "Uzinduzi wa Programu ya Uoteshaji Miti 2025",
            excerpt: "Better Life inatangaza uzinduzi wa mpango wake kabambe wa kupanda miti milioni 1 kufikia 2025.",
          },
          unepPartnership: {
            title: "Ushirikiano wa Kimkakati na UNEP",
            excerpt: "Kusainiwa kwa makubaliano ya ushirikiano na Mpango wa Mazingira wa Umoja wa Mataifa ili kuimarisha matendo yetu.",
          },
          farmersTraining: {
            title: "Mafunzo ya Wakulima 500 huko Kikwit",
            excerpt: "Mafanikio ya programu yetu ya mafunzo ya kilimo kinachozingatia tabianchi tukiwa na wakulima 500 waliofunzwa.",
          },
          bioCertification: {
            title: "Cheti cha Kilimo Hai kwa Wazalishaji 150",
            excerpt: "Wazalishaji 150 wanaosaidiwa na Better Life wapata cheti chao cha kilimo hai.",
          },
          nurseryMbandaka: {
            title: "Kituo Kipya cha Miche huko Mbandaka",
            excerpt: "Uzinduzi wa kituo chetu kipya cha miche chenye uwezo wa kutoa miche 600,000 kwa mwaka.",
          },
          excellenceAward: {
            title: "Tuzo ya Ubora wa Mazingira 2024",
            excerpt: "Better Life yapata Tuzo ya Ubora wa Mazingira kutoka Wizara ya Mazingira.",
          },
        },
      },
      gallery: {
        hero: {
          title: "Matunzio ya Picha",
          subtitle: "Gundua vitendo vyetu nyanjani na mafanikio katika picha",
        },
        intro: {
          kicker: "Picha Zetu",
          title: "Gundua Kazi Yetu",
          text: "Picha na video za miradi yetu, matukio, na mafanikio katika maeneo tofauti ya DRC.",
        },
        stats: {
          photos: "Picha",
          videos: "Video",
          albums: "Albamu",
          years: "Miaka ya kumbukumbu",
        },
        categories: {
          title: "Vitengo",
          all: "Zote",
          projects: "Miradi",
          events: "Matukio",
          communities: "Jamii",
          nature: "Asili",
        },
        filters: {
          title: "Chuja",
          date: "Tarehe",
          category: "Kitengo",
          location: "Mahali",
        },
        collections: {
          kicker: "Mikusanyiko",
          title: "Albamu za Kimaudhui",
          viewAlbum: "Angalia albamu",
          photos: "picha",
          reforestation: {
            title: "Uoteshaji Miti 2024",
            desc: "Picha za kampeni zetu za upandaji miti nchi nzima",
          },
          farmersTraining: {
            title: "Mafunzo ya Wakulima",
            desc: "Vivutio vya vikao vyetu vya mafunzo ya kilimo endelevu",
          },
          biodiversity: {
            title: "Bioanuwai",
            desc: "Viumbe wa porini na mimea inayolindwa na programu zetu za uhifadhi",
          },
          communities: {
            title: "Jamii",
            desc: "Mikutano na jamii zinazofaidika na miradi yetu",
          },
          nurseries: {
            title: "Vituo vya Miche",
            desc: "Vituo vyetu vya miche shuleni na uzalishaji wa miche",
          },
          events: {
            title: "Matukio",
            desc: "Sherehe, uzinduzi, na maadhimisho ya mafanikio yetu",
          },
        },
        recent: {
          kicker: "Zilizotokea Hivi Punde",
          title: "Picha za Hivi Punde Zilizoongezwa",
          items: {
            plantationKinshasa: "Upandaji wa Kinshasa",
            trainingKikwit: "Mafunzo ya Kikwit",
            modernHives: "Mizinga ya Kisasa",
            villageWell: "Kisima cha Kijiji",
            certificates: "Sherehe za Vyeti",
            bioHarvest: "Mavuno ya Kijani",
            butterfliesSalonga: "Vipepeo wa Salonga",
            schoolGarden: "Bustani ya Shule",
            unepPartnership: "Ushirikiano wa UNEP",
            excellenceAward: "Tuzo ya Ubora",
            nurseryMbandaka: "Kituo cha Miche cha Mbandaka",
            communityAssembly: "Mkusanyiko wa Jamii",
          },
        },
        videos: {
          kicker: "Video",
          title: "Filamu Zetu za Kumbukumbu",
          watch: "▶ Tazama",
          duration: "Muda",
          views: "Mitazamo",
          items: {
            reforestationYear: "Uoteshaji Miti: Mwaka Mmoja wa Upandaji",
            farmersPortrait: "Picha ya Wakulima Waliobadilishwa",
            congoBiodiversity: "Bioanuwai ya Bonde la Kongo",
          },
        },
        empty: {
          title: "Hakuna Picha",
          text: "Hakuna picha zinazopatikana kwa wakati huu. Tafadhali rudia baadaye kwa maudhui mapya.",
        },
        view: {
          title: "Angalia Picha",
          close: "Funga",
          prev: "Iliyopita",
          next: "Inayofuata",
          info: "Habari",
        },
        cta: {
          title: "Shiriki Picha Zako",
          text: "Je, ulishiriki katika moja ya matukio yetu? Shiriki picha zako nasi!",
          btn: "Tuma Picha",
        },
      },
    },

    communautaire: {
      energie: {
        hero: {
          title: "Nishati Endelevu",
          subtitle: "Nishati safi kwa maisha bora"
        }
      }
    },

    footer: {
      location: "Joli Parc, C/Ngaliema",
      tagline: "Maendeleo Endelevu kwa Vizazi Vijavyo",
      description: "Ulinzi wa Mazingira, Bioanuwai na Maendeleo Endelevu katika Jamhuri ya Kidemokrasia ya Kongo.",
      nav_title: "Maelekezo",
      contact_title: "Mawasiliano",
      rights: "Haki zote zimehifadhiwa  Better Life",
      activities_title: "Shughuli",
      newsletter: {
        title: "Jiandikishe kwa Jarida Letu",
        placeholder: "Barua pepe yako",
        button: "Jiandikisha"
      },
      quick_links: "Viungulo Haraka",
      contact_info: "Mawasiliano",
      social_media: "Mitandao ya Kijamii"
    },
    donation: {
      hero: {
        title: "Toa Msaada",
        subtitle: "Saidia matendo yetu kwa ajili ya mustakabali endelevu."
      },
      kicker: "Msaada",
      title: "Msaada Wako ni wa Thamani",
      description: "Kila mchango unatuwezesha kupanua programu zetu na kuwa na athari halisi mashinani.",
      support: {
        title: "Kwa nini utusaidie?",
        text: "Michango yako inafadhili moja kwa moja upandaji miti, kilimo endelevu, na msaada kwa jamii."
      },
      rights: "Haki zote zimehifadhiwa © Better Life",
      activities_title: "Shughuli",
      newsletter: {
        title: "Jiandikishe kwa Jarida Letu",
        placeholder: "Barua pepe yako",
        button: "Jiandikisha"
      },
      quick_links: "Viungulo Haraka",
      contact_info: "Mawasiliano",
      social_media: "Mitandao ya Kijamii"
    }
  },
  testimonialSection: {
    title: "Ushuhuda",
    subtitle: "Gundua hadithi za mabadiliko na athari kutoka kwa washirika na wanufaika wetu",
    items: [
      {
        id: 1,
        name: "KABAMBA KABAMBA Chrétien",
        shortQuote: "Kupitia Better Life, tunasaidia kilimo endelevu, upandaji miti na maendeleo ya kijamii katika maeneo kadhaa ya Kongo. Miradi yetu inalenga kuboresha hali ya maisha ya jamii, hasa upatikanaji wa elimu na huduma za afya.",
        fullQuote: "Tumeona hali ngumu ya maisha katika maeneo mengi ya Kongo: ukosefu wa maji safi, shule zisizotosheleza, upatikanaji mdogo wa huduma za afya. Kukabiliana na changamoto hizi, tunafanya utetezi ili wafadhili wa kitaifa na kimataifa wasaidie miradi yetu inayolenga kuboresha maendeleo ya kijamii na kiuchumi ya jamii, vijijini na mijini.\n\nTukiwa na Better Life, tunaingilia kati katika nyanja kadhaa: kilimo endelevu (mazao ya karanga), upandaji miti, ulinzi wa maeneo ya ardhi oevu na uhamasishaji wa fedha. Njia hii inayoweza kutofautiana inatuwezesha kujibu kwa vitendo mahitaji ya idadi ya watu na kusaidia mipango inayoweza kudumu.",
        location: "Lomami"
      },
      {
        id: 2,
        name: "MWAMBA SELEMANI Gérard",
        shortQuote: "Huko Moubambé, tulizindua mradi wa \"Watoto Sifuri katika Migodi\", tukichanganya kilimo endelevu na upatikanaji wa elimu. Kupitia Better Life, watoto wanaondoka migodini kwenda kusoma na jamii inafuata mbinu endelevu.",
        fullQuote: "Huko Moubambé, eneo la mbali la Haut-Katanga, tulianzisha mradi wa kilimo endelevu kwa ushirikiano na Better Life. Idadi ya watu, ambao hapo awali walitegemea uchimbaji mdogo wa madini, walihamasishwa kuhusu kilimo endelevu. Tuliweka vitalu na kuhamasisha jamii ya mahali hapo ili watoto waweze kuondoka migodini na kupata elimu.\n\nMradi wa \"Watoto Sifuri katika Migodi\" pia unatoa fursa ya ujenzi wa shule na vituo vya mafunzo ya ufundi kwa vijana na watu wazima. Ushuhuda huu unaonyesha jinsi ahadi ya pamoja na usaidizi wa kiufundi unavyoweza kubadilisha maisha ya jamii za vijijini kwa namna endelevu.",
        location: "Haut-Katanga"
      }
    ],
    btn_video: "Tazama Video",
    btn_more: "Soma zaidi",
    btn_less: "Soma kidogo",
    btn_more_section: "Tazama ushuhuda zaidi",
    modal: {
      title: "Ushuhuda wa Video",
      info_title: "Ushuhuda wa Better Life",
      info_text: "Gundua athari halisi ya miradi yetu kupitia ushuhuda wa washirika na wanufaika wetu."
    }
  }
};

export default sw;
