...
function Shell({ children }) {
  ...
  return (
    <div className="bg-slate-50 text-slate-900">
      <div
        className={`fixed top-0 z-30 w-full transition-transform duration-200 ${showTopBar ? 'translate-y-0' : '-translate-y-full'}`}
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <Link to="/" className="flex items-center gap-2">
            <img src={logoOfficial} alt="Better Life" className="h-10 w-auto" />
          </Link>
          ...
          <Link
            to="/contact"
            className="ml-3 inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-semibold text-white shadow-sm transition"
      ...
      <footer className="border-t border-slate-200" style={{ backgroundColor: '#0f70b7' }}>
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-6 py-6 text-sm text-white">
          <div className="flex items-center gap-3">
            <img src={logoWhite} alt="Better Life" className="h-10 w-auto" />
            <p className="text-white/80">Construire un impact durable.</p>
          </div>
          <p className="text-white/70">© {new Date().getFullYear()} Better Life</p>
        </div>
      </footer>
    </div>
  )
