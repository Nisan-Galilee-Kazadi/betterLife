...
const { getHeroSlidesCreateTableSql, seedHeroSlidesIfEmpty } = require('./heroSlidesSeed');
...
function getMailRuntimeConfig(settings = {}) {
    ...
    const fromAddress = String(process.env.MAIL_FROM || user || settings.company_email || '').trim();
    ...
    const port = Number(process.env.MAIL_PORT || (service.toLowerCase() === 'gmail' ? 465 : 587)) || 0;
    const secure = parseBooleanEnv(process.env.MAIL_SECURE, port === 465);
    const fromName = String(process.env.MAIL_FROM_NAME || settings.site_name || 'Better Life').trim();
    ...
    const isConfigured = !!user && !!pass && (!!service || !!host) && !!fromAddress;
...
async function ensureIndex(tableName, indexName, definition) {
    try {
        ...
        console.warn(`[WARN] Could not ensure index ${indexName} on ${tableName}:`, e.message);
    }
}
...
function buildContactMessageIdentity(row = {}) {
    return {
        ...
        message_uid: typeof row.message_uid === 'string' ? row.message_uid.trim() : null,
    };
}
...
function parseLegacyContactMessageReference(reference = '') {
    ...
    try {
        const payload = JSON.parse(Buffer.from(String(reference).slice(7), 'base64url').toString('utf8'));
...
function buildContactMessageLookup(row = {}) {
    ...
    return {
        clause: 'id <=> ? AND name <=> ? AND email <=> ? AND phone <=> ? AND subject <=> ? AND message <=> ? AND created_at <=> ?',
        params: [
            identity.id,
            identity.name,
            identity.email,
...
function buildReplyEmailText(contactMessage, replyMessage, siteName, replyToAddress) {
    const lines = [
...
function buildReplyEmailHtml(contactMessage, replyMessage, siteName, replyToAddress) {
    return `
...
async function sendContactReplyEmail(contactMessage, replyMessage) {
    ...
    if (!contactMessage?.email) {
        const error = new Error("Le message selectionne n'a pas d'adresse email valide.");
    ...
    const mailOptions = {
        from: formatMailbox(mailRuntime.config.fromName, mailRuntime.config.fromAddress),
        to: contactMessage.email,
        replyTo: formatMailbox(mailRuntime.config.replyToName, mailRuntime.config.replyToAddress),
        subject,
        text: buildReplyEmailText(contactMessage, replyMessage, siteName, mailRuntime.config.replyToAddress),
        html: buildReplyEmailHtml(contactMessage, replyMessage, siteName, mailRuntime.config.replyToAddress),
    };

    const info = await mailRuntime.transporter.sendMail(mailOptions);
    return {
...
async function ensureContactMessageUids() {
    try {
        ...
        for (const row of rows) {
            const currentUid = typeof row.message_uid === 'string' ? row.message_uid.trim() : '';
...
async function initializeDatabase() {
    try {
        ...
        const [settingsRows] = await db.execute('SELECT COUNT(*) as count FROM settings');
        if (settingsRows[0].count === 0) {
            await db.execute('INSERT INTO settings (site_name) VALUES ("Better Life")');
        }
        ...
        await ensureColumn('career_applications', 'offer_title', 'VARCHAR(255) NULL');
        ...
        await ensureColumn('contact_messages', 'is_favorite', 'BOOLEAN DEFAULT FALSE');
        await ensureColumn('contact_messages', 'is_archived', 'BOOLEAN DEFAULT FALSE');
        ...
        await ensureColumn('contact_messages', 'email_message_id', 'VARCHAR(255) NULL');
        await ensureColumn('contact_messages', 'email_reply_to', 'VARCHAR(255) NULL');
        await ensureColumn('contact_messages', 'email_delivery_status', 'VARCHAR(30) NULL');
        await ensureColumn('contact_messages', 'email_delivery_details', 'TEXT NULL');
        ...
        await ensureColumn('settings', 'top_bar_tagline_fr', "VARCHAR(255) DEFAULT 'Votre partenaire pour un avenir meilleur'");
        await ensureColumn('settings', 'top_bar_tagline_en', "VARCHAR(255) DEFAULT 'Your partner for a better future'");
        await ensureColumn('settings', 'top_bar_tagline_es', "VARCHAR(255) DEFAULT 'Su socio para un futuro mejor'");
        await ensureColumn('settings', 'top_bar_tagline_sw', "VARCHAR(255) DEFAULT 'Mshirika wako kwa mustakabali bora'");
        ...
        try {
            await db.execute("UPDATE statistics SET label_fr = label WHERE label_fr IS NULL AND label IS NOT NULL");
        } catch (e) { /* ignore */ }
...
apiRouter.post('/login', async (req, res) => {
    ...
    if (username === ADMIN_USER && password === ADMIN_PASS) {
        ...
        return res.json({ success: true, token, user: { username, role: 'admin', display_name: 'Admin BetterLife' } });
    }
    ...
    try {
        ...
        for (const row of teamRows) {
            ...
            if (username === expectedUsername && password === expectedPassword) {
                ...
                const token = jwt.sign({ username, role: roleKey, source_type: 'team', source_id: row.id }, JWT_SECRET, { expiresIn: '8h' });
                return res.json({
                    success: true,
                    token,
                    user: {
        ...
        for (const row of delegateRows) {
            ...
            const expectedPassword = buildPanelDefaultPassword({ ...entry }, 'delegate');
            if (username === expectedUsername && password === expectedPassword) {
                const token = jwt.sign({ username, role: 'delegue', source_type: 'delegate', source_id: row.id }, JWT_SECRET, { expiresIn: '8h' });
                return res.json({
                    success: true,
                    token,
                    user: {
...
function buildPanelUsername(entry, sourceType) {
    if (sourceType === 'delegate') {
        const provinceToken = normalizePanelAccessToken(entry.province || entry.display_name || entry.email || '');
    ...
    const emailToken = normalizePanelAccessToken(String(entry.email || '').split('@')[0]);
...
function serializePanelUser(entry, sourceType) {
    ...
    return {
        ...
        username: buildPanelUsername({ ...entry, role_label: roleLabel }, sourceType),
        default_password: buildPanelDefaultPassword({ ...entry, role_label: roleLabel }, sourceType),
        email: entry.email || '',
        phone: entry.phone || '',
...
apiRouter.get('/users', async (req, res) => {
    try {
        ...
        if (requestUsername !== ADMIN_USER) {
            return res.status(403).json({ error: 'Acces reserve au compte admin principal' });
        }
...
apiRouter.put('/settings', async (req, res) => {
    try {
        const allowed = [
            'site_name', 'site_description', 'company_email', 'company_phone', 'company_address',
            'facebook_url', 'twitter_url', 'linkedin_url', 'instagram_url', 'youtube_url',
            'whatsapp_url', 'tiktok_url', 'logo_path', 'theme',
            'top_bar_tagline_fr', 'top_bar_tagline_en', 'top_bar_tagline_es', 'top_bar_tagline_sw'
        ];
        const updates = [];
        const values = [];
        ...
        if (updates.length > 0) {
            values.push(1);
            await db.execute(`UPDATE settings SET ${updates.join(', ')} WHERE id = ?`, values);
        }
...
function normalizeHeroSlidePayload(payload = {}, existing = null) {
    const next = {
        ...
        title_fr: payload.title_fr !== undefined
            ? normalizeRequiredHeroValue(payload.title_fr, 'title_fr')
            : normalizeRequiredHeroValue(existing?.title_fr, 'title_fr'),
        title_en: payload.title_en !== undefined ? normalizeOptionalHeroValue(payload.title_en) : existing?.title_en ?? null,
        title_es: payload.title_es !== undefined ? normalizeOptionalHeroValue(payload.title_es) : existing?.title_es ?? null,
        title_sw: payload.title_sw !== undefined ? normalizeOptionalHeroValue(payload.title_sw) : existing?.title_sw ?? null,
        description_fr: payload.description_fr !== undefined ? normalizeOptionalHeroValue(payload.description_fr) : existing?.description_fr ?? null,
        description_en: payload.description_en !== undefined ? normalizeOptionalHeroValue(payload.description_en) : existing?.description_en ?? null,
        description_es: payload.description_es !== undefined ? normalizeOptionalHeroValue(payload.description_es) : existing?.description_es ?? null,
        description_sw: payload.description_sw !== undefined ? normalizeOptionalHeroValue(payload.description_sw) : existing?.description_sw ?? null,
        is_active: payload.is_active !== undefined ? !!payload.is_active : !!existing?.is_active,
    };

    next.alt_fr = payload.alt_fr !== undefined
        ? normalizeOptionalHeroValue(payload.alt_fr)
        : existing?.alt_fr ?? null;
    next.alt_en = payload.alt_en !== undefined
        ? normalizeOptionalHeroValue(payload.alt_en)
        : existing?.alt_en ?? null;
    next.alt_es = payload.alt_es !== undefined
        ? normalizeOptionalHeroValue(payload.alt_es)
        : existing?.alt_es ?? null;
    next.alt_sw = payload.alt_sw !== undefined
...
async function reorderHeroSlide(connection, slideId, requestedPosition) {
    ...
    const targetIndex = clampHeroSlidePosition(requestedPosition, orderedIds.length + 1) - 1;
...
async function insertHeroSlide(connection, slide, requestedPosition) {
    ...
    const targetPosition = clampHeroSlidePosition(requestedPosition, rows.length + 1);
...
apiRouter.get('/hero-slides/admin', async (req, res) => {
    try {
        setNoStore(res);
        const [rows] = await db.execute('SELECT * FROM hero_slides ORDER BY position ASC, id ASC');
...
apiRouter.post('/hero-slides', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, id: created.id, position: created.position, created: true });
    } catch (error) {
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(req.body || {}, 'position')) {
            ...
            if (Number.isFinite(requestedPosition) && requestedPosition > 0) {
                nextPosition = await reorderHeroSlide(connection, slideId, requestedPosition);
            }
        }
...
apiRouter.delete('/hero-slides/:id', async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        await connection.execute('DELETE FROM hero_slides WHERE id = ?', [req.params.id]);
...
function normalizeStatisticPayload(payload = {}, existing = null, fallbackStatKey = null) {
    const statKey = String(payload.stat_key ?? fallbackStatKey ?? existing?.stat_key ?? '').trim();
    ...
    return {
        ...
        color: String(payload.color ?? existing?.color ?? 'green').trim() || 'green',
        icon: String(payload.icon ?? existing?.icon ?? 'FaUsers').trim() || 'FaUsers',
        is_active: payload.is_active !== undefined ? !!payload.is_active : existing?.is_active !== false,
    };
}
...
apiRouter.get('/statistics', async (req, res) => {
    try {
        ...
        res.json(sortStatistics(rows).map((row) => ({
            ...row,
            is_active: !!row.is_active,
        })));
    } catch (error) { res.status(500).json({ error: error.message }); }
...
apiRouter.put('/statistics/:id', async (req, res) => {
    try {
        ...
        const nextStatistic = normalizeStatisticPayload(req.body || {}, existing, statKey);
...
apiRouter.delete('/statistics/:id', async (req, res) => {
    try {
        await db.execute('DELETE FROM statistics WHERE stat_key = ?', [req.params.id]);
        res.json({ success: true });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/testimonials', async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`SELECT * FROM testimonials ${where} ORDER BY sort_order ASC, created_at DESC`);
        res.json(rows.map(t => ({
            ...t,
...
apiRouter.get('/testimonials/admin', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM testimonials ORDER BY sort_order ASC, created_at DESC');
        res.json(rows);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/testimonials/:id', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM testimonials WHERE id = ? LIMIT 1', [req.params.id]);
        if (!rows || rows.length === 0) return res.status(404).json({ error: 'Testimonial not found' });
        res.json(rows[0]);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.put('/testimonials/:id', async (req, res) => {
    try {
        const allowed = [
            ...
            'image_path', 'video_url', 'rating', 'is_featured', 'sort_order', 'is_active',
        ];
        const updates = [];
        const values = [];
        for (const field of allowed) {
            if (Object.prototype.hasOwnProperty.call(req.body || {}, field)) {
                ...
                else if (field === 'rating' || field === 'sort_order') value = value === '' || value == null ? null : Number(value);
                updates.push(`${field} = ?`);
                values.push(value);
            }
        }
        ...
        await db.execute(`UPDATE testimonials SET ${updates.join(', ')} WHERE id = ?`, values);
        res.json({ success: true });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.post('/testimonials', async (req, res) => {
    try {
        const { name, location, role, short_quote_fr, full_quote_fr, image_path, video_url, rating = 5, is_featured = false, is_active = true, sort_order = 0 } = req.body;
        const [result] = await db.execute(
            `INSERT INTO testimonials (name, location, role, short_quote_fr, full_quote_fr, image_path, video_url, rating, is_featured, is_active, sort_order)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [name, location || null, role || null, short_quote_fr || null, full_quote_fr || null, image_path || null, video_url || null, rating, !!is_featured, !!is_active, sort_order]
        );
        res.json({ success: true, id: result.insertId });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
function normalizePagePayload(payload = {}, { currentPageId = null } = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'meta_description')) {
        normalized.meta_description_fr = normalizePageFieldValue(payload.meta_description);
    }
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'sort_order')) {
        ...
        normalized.sort_order = Number.isFinite(sortOrder) ? Math.trunc(sortOrder) : 0;
    }
    if (Object.prototype.hasOwnProperty.call(payload, 'parent_id')) {
        const parentId = Number(payload.parent_id);
        normalized.parent_id = Number.isFinite(parentId) && parentId > 0 && parentId !== Number(currentPageId)
            ? parentId
            : null;
    }
...
apiRouter.get('/pages/slug/:slug', async (req, res) => {
    try {
        const lang = req.query.lang || 'fr';
        const [rows] = await db.execute('SELECT * FROM pages WHERE LOWER(slug) = LOWER(?) LIMIT 1', [req.params.slug]);
...
apiRouter.put('/pages/:id', async (req, res) => {
    try {
        const allowedPayload = normalizePagePayload(req.body || {}, { currentPageId: req.params.id });
        const updates = [];
        const values = [];
        ...
        values.push(req.params.id);
        await db.execute(`UPDATE pages SET ${updates.join(', ')} WHERE id = ?`, values);
        res.json({ success: true });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.post('/pages', async (req, res) => {
    try {
        ...
        if (!slug || !titleFr) {
            return res.status(400).json({ error: 'slug et title_fr sont obligatoires' });
        }
        const [result] = await db.execute(
            `INSERT INTO pages (
...
apiRouter.get('/partners', async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`SELECT * FROM partners ${where} ORDER BY sort_order ASC, id ASC`, values);
        res.json(rows);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/partners/admin', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM partners ORDER BY sort_order, id');
        res.json(rows);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/team', async (req, res) => {
    try {
        ...
        res.json(rows.map(m => ({
            ...
            email: m.email, photo_path: m.photo_path,
...
apiRouter.get('/delegates', async (req, res) => {
    try {
        ...
        res.json(rows.map(d => ({
            id: d.id, name: d.name, province: d.province, role: d.role,
            bio: d[`bio_${lang}`] || d.bio_fr,
            email: d.email, photo_path: d.photo_path,
...
apiRouter.post('/contact-messages', async (req, res) => {
    try {
        ...
        res.json({ success: true, id: result.insertId, message: serializeContactMessage(createdMessage) });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function getContactMessageByReference(messageRef) {
    ...
    if (/^\d+$/.test(reference) && reference !== '0') {
        const [rows] = await db.execute('SELECT * FROM contact_messages WHERE id = ? LIMIT 1', [Number(reference)]);
        return rows[0] || null;
    }

    const [rows] = await db.execute('SELECT * FROM contact_messages WHERE message_uid = ? LIMIT 1', [reference]);
    return rows[0] || null;
}
...
async function updateContactMessageFields(messageRef, fields = {}) {
    ...
    const updatedMessage = await getContactMessageByReference(getContactMessageReference(targetMessage));
    return serializeContactMessage(updatedMessage);
}

async function updateContactMessageBooleanFlag(messageRef, fieldName, requestedValue) {
    const targetMessage = await getContactMessageByReference(messageRef);
    if (!targetMessage) {
        return null;
    }
...
async function saveContactMessageReply(req, res) {
    ...
    try {
        const targetMessage = await getContactMessageByReference(req.params.messageRef);
        if (!targetMessage) {
            return res.status(404).json({ error: 'Message not found' });
        }

        const delivery = await sendContactReplyEmail(targetMessage, replyMessage);
        const updatedMessage = await updateContactMessageFields(req.params.messageRef, {
            ...
            email_delivery_status: delivery.rejected?.length ? 'rejected' : 'accepted',
...
apiRouter.put('/contact-messages/:messageRef/read', async (req, res) => {
    try {
        const updatedMessage = await updateContactMessageFields(req.params.messageRef, { is_read: true });
...
apiRouter.put('/contact-messages/:messageRef', async (req, res) => {
    try {
        const updatedMessage = await updateContactMessageFields(req.params.messageRef, req.body || {});
        if (!updatedMessage) return res.status(404).json({ error: 'Message not found' });
        res.json({ success: true, message: updatedMessage });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
apiRouter.delete('/contact-messages/:messageRef', async (req, res) => {
    try {
        ...
        const lookup = buildContactMessageLookup(targetMessage);
        await db.execute(`DELETE FROM contact_messages WHERE ${lookup.clause} LIMIT 1`, lookup.params);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
apiRouter.post(
    ...
    async (req, res) => {
        try {
            ...
            res.json({ success: true, id: result.insertId, cv_path: cvPath, letter_path: letterPath });
        } catch (e) {
...
apiRouter.get('/career-applications/admin', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM career_applications ORDER BY created_at DESC, id DESC');
        res.json(rows);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
apiRouter.delete('/career-offers/:id', async (req, res) => {
    try {
        ...
        await db.execute('DELETE FROM career_applications WHERE offer_id = ?', [offerId]);
...
apiRouter.get('/gallery/admin', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM gallery_items ORDER BY sort_order ASC, created_at DESC');
...
apiRouter.get('/gallery/:id', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM gallery_items WHERE id = ?', [req.params.id]);
...
apiRouter.post('/gallery', async (req, res) => {
    try {
        const { type = 'photo', category = 'general', title_fr, title_en, title_es, title_sw,
            description_fr, description_en, description_es, description_sw,
            image_url, video_url, video_duration, is_active = true, sort_order = 0 } = req.body;
        ...
        const [result] = await db.execute(
            `INSERT INTO gallery_items (type, category, title_fr, title_en, title_es, title_sw,
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [type, category, title_fr, title_en, title_es, title_sw,
                description_fr, description_en, description_es, description_sw,
                image_url, video_url, video_duration, !!is_active, sort_order]
        );
        res.json({ success: true, id: result.insertId });
...
apiRouter.put('/gallery/:id', async (req, res) => {
    try {
        const allowed = ['type', 'category', 'title_fr', 'title_en', 'title_es', 'title_sw',
            'description_fr', 'description_en', 'description_es', 'description_sw',
            'image_url', 'video_url', 'video_duration', 'is_active', 'sort_order'];
        const updates = [];
        const values = [];
        ...
        await db.execute(`UPDATE gallery_items SET ${updates.join(', ')} WHERE id = ?`, values);
        res.json({ success: true });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/about-content/admin', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM about_content ORDER BY section ASC, sort_order ASC');
        res.json(rows.map(item => ({ ...item, is_active: !!item.is_active })));
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
apiRouter.get('/about-content/:id', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT * FROM about_content WHERE id = ?', [req.params.id]);
...
apiRouter.post('/about-content', async (req, res) => {
    try {
        const { section, key_name, title_fr, title_en, title_es, title_sw,
            content_fr, content_en, content_es, content_sw,
            icon, image_url, sort_order = 0, is_active = true } = req.body;

        if (!section || !key_name) {
            return res.status(400).json({ error: 'section and key_name are required' });
        }
        ...
        const [result] = await db.execute(
            `INSERT INTO about_content (section, key_name, title_fr, title_en, title_es, title_sw,
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [section, key_name, title_fr, title_en, title_es, title_sw,
                content_fr, content_en, content_es, content_sw,
                icon, image_url, sort_order, !!is_active]
        );
        res.json({ success: true, id: result.insertId });
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ error: 'Content with this section and key_name already exists' });
        }
...
apiRouter.put('/about-content/:id', async (req, res) => {
    try {
        const allowed = ['section', 'key_name', 'title_fr', 'title_en', 'title_es', 'title_sw',
            'content_fr', 'content_en', 'content_es', 'content_sw',
            'icon', 'image_url', 'sort_order', 'is_active'];
        const updates = [];
        const values = [];
        ...
        await db.execute(`UPDATE about_content SET ${updates.join(', ')} WHERE id = ?`, values);
        res.json({ success: true });
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
            return res.status(409).json({ error: 'Content with this section and key_name already exists' });
